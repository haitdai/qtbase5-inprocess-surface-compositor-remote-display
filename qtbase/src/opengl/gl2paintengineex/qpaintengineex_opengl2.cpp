...
//DHT 20150202 注意cpu 光栅化glyph 和 gpu 光栅化glyph 的不同条件
void QGL2PaintEngineEx::drawTextItem(const QPointF &p, const QTextItem &textItem)
{
    ...
    //cpu 光栅化(并cache)的条件:  QT_MAX_CACHED_GLYPH_SIZE
    if (shouldDrawCachedGlyphs(ti.fontEngine, s->matrix)) {
...

        {
            ...
            d->drawCachedGlyphs(glyphType, &staticTextItem);         //DHT  cpu 光栅化glyph,  并 cache 到texture
        }
        return;
    }

    //DHT fall back to path drawing(use opengl es2 paintengine),  GPU 光栅化, 不cache
...
//DHT 20150202
//freetype2 支持4种glyph:
//composite, bitmap, outline, plotter
//由cpu 光栅化glyph 并填充bits (光栅化的输出) 到cached texture
                                                ...
                                                QStaticTextItem *staticTextItem)
{
    ...
    if (recreateVertexArrays) {
        ...
        cache->fillInPendingGlyphs();       //<<<< DHT  cpu 光栅化 glyph 并填充到cache texture
    }
