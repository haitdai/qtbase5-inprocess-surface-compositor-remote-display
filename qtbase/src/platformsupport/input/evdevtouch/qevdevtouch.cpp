...
public:
    ...
    void assignIds();
    void updateTransform(Qt::ScreenOrientation newOrientation);  /*> DHT */
    ...
    QTransform m_rotate;
    bool m_singleTouch;
    Qt::ScreenOrientation      scrnOrient;                  /*< DHT cache screen orientation to avoid transforming coordinates every touch event */    
};
...
/*
    \brief update touch screen rotation transform(matrix) according to screen NATIVE orientation,
    QScreen::orientation() is NOT work since influnced by QScreen::orientationUpdateMask() which is intend to be set by application, 
    like dubhe_frontend, not by Qt itself.
    \note, the rotation direction(clockwise/counterclockwise) is just opposite between QTransform and PVRTMat4.
*/
void QEvdevTouchScreenData::updateTransform(Qt::ScreenOrientation newOrientation)
{
    if (Qt::PortraitOrientation ==  newOrientation) {
        this->m_rotate = QTransform::fromTranslate(0.5, 0.5).rotate(0).translate(-0.5, -0.5);
    } else if(Qt::LandscapeOrientation ==  newOrientation) {
        this->m_rotate = QTransform::fromTranslate(0.5, 0.5).rotate(90).translate(-0.5, -0.5);
    } else if(Qt::InvertedPortraitOrientation ==  newOrientation) {
        this->m_rotate = QTransform::fromTranslate(0.5, 0.5).rotate(180).translate(-0.5, -0.5);  
    } else if(Qt::InvertedLandscapeOrientation ==  newOrientation) {
        this->m_rotate = QTransform::fromTranslate(0.5, 0.5).rotate(-90).translate(-0.5, -0.5);
    } else {
        //fall through under other conditions, such as Qt::PrimaryOrientation
    }
}
...
#endif
{
    ...
    d->scrnOrient = QGuiApplication::primaryScreen()->orientation();
    d->updateTransform(d->scrnOrient);
...
void QEvdevTouchScreenData::addTouchPoint(const Contact &contact, Qt::TouchPointStates *combinedStates)
{
    ...
    *combinedStates |= tp.state;
    ...
    // since signal/handler is not for me, peek screen orientation from primary every touch event
    //TODO: multi-screen consideration
    Qt::ScreenOrientation orient = QGuiApplication::primaryScreen()->nativeOrientation();
    if (orient != this->scrnOrient) { /*< do the transform */
        updateTransform(orient);
        this->scrnOrient = orient;
    } //m_rotate is OK now ...
