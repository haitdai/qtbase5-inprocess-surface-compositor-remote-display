...
QDirectFbBlitter::QDirectFbBlitter(const QSize &rect, IDirectFBSurface *surface)
    : QBlittable(rect, dfb_blitter_capabilities())               //DHT 20141230, direct fb capabilities
        , m_surface(surface)
{
...
//DHT DirectFB 貌似只支持 Source 和 SourceOver 方式的合成OP
...
//DHT 20141230 lock 出 directfb surface 的内存，供cpu读写(direct read/write),  cpu读写时不可加速；
//外部blitter 读写可以加速(QBlitterPaintEngine/QDirectFBBlitter)，此时为unlocked 状态
