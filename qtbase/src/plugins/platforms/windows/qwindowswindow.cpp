...
#ifdef QT_MROFS
#include "mrofs/qmrofscompositor.h"       // QMrOfsCompositor
#include "qwindowsnativeimage.h"    // QWindowsNativeImage
#endif
...
QWindowsWindow::QWindowsWindow(QWindow *aWindow, const WindowData &data) :
    ...
    m_iconBig(0)
#ifdef QT_MROFS
    ,m_compositor(NULL), m_image_shadow(NULL)
#endif
{
...
#ifdef QT_MROFS
    destroyShadowImage();
    createShadowImage(data.geometry);
#endif    
}
...
void QWindowsWindow::setGeometry(const QRect &rectIn)
{
    ...
    if (m_data.hwnd) {
        ...
        QPlatformWindow::setGeometry(rect);     // DHT defer set
    }

#ifdef QT_MROFS
    if(m_compositor && newSize != oldSize) {
        static_cast<QMrOfsCompositor*>(m_compositor)->setWindowGeometry(rect);  // exclude frame margins
    }
#endif
}
...
#ifdef QT_MROFS
void QWindowsWindow::setCompositor(QPlatformCompositor *plat_compositor)
{
    m_compositor = plat_compositor;
}

QPlatformImage *QWindowsWindow::createShadowImage(const QRect rect)
{
    if(m_image_shadow)
        delete m_image_shadow;
        
    QImage::Format format = QWindowsNativeImage::systemFormat();
    if (format == QImage::Format_RGB32 && this->window()->format().hasAlpha())
        format = QImage::Format_ARGB32_Premultiplied;
    m_image_shadow = new QWindowsNativeImage(rect.width(), rect.height(), format);

    return m_image_shadow;
}

void QWindowsWindow::destroyShadowImage()
{
    if(m_image_shadow) {
        delete m_image_shadow;
        m_image_shadow = NULL;
...
QPlatformImage *QWindowsWindow::shadowImage()
{
    return m_image_shadow;
}
#endif
