...
//DHT 20150126
// -platform xcb[[:<host>]:<major.minor>]...  -display [<host>]:<default_major.default_minor>
//<X>
    ...
    , m_instanceName(0)
{
    ...
    if (argc) {
        ...
        for (int i = 1; i < argc; i++) {
            char *arg = argv[i];
            if (arg) {
                if (!strcmp(arg, "-display") && i < argc - 1) {             //DHT  which display frontend connect to ...
                    displayName = argv[++i];
                    arg = 0;
    ...
    m_connections << new QXcbConnection(m_nativeInterface.data(), canGrab, displayName);   //DHT  handle -display ...

    for (int i = 0; i < parameters.size() - 1; i += 2) {                                                             //DHT handle -xcb ..., ref init_platform in qguiapplication.cpp
#ifdef Q_XCB_DEBUG
        //DHT 这里的设计意图是一个client 同时连接多个server(这些servers可同时显示client内容并投递input给client), 目前没有这样的使用方式
