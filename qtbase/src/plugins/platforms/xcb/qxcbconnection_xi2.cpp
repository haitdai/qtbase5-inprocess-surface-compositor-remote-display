...
//DHT 20140127 xi2 for multitouch of xorg, no use under bigdipper
...
void QXcbConnection::xi2Select(xcb_window_t window)
{
    ...
    foreach (XInput2DeviceData *dev, m_touchDevices.values()) {
        ...
        if (m_xi2Minor >= 2 && result == Success) {
            ...
            qWarning("XISelectEvents OK: %d", result);            
        } else {
            qWarning("XISelectEvents FAIL: %d", result);
        }
    }
...
XInput2DeviceData *QXcbConnection::deviceForId(int id)
{
    ...
    if (!dev) {
        ...
        for (int i = 0; i < dev->xiDeviceInfo->num_classes; ++i) {
            ...
            //DHT, 20141020, if Xorg-evdev does NOT support MULTITOUCH, (xf86-input-evdev-2.6.0)
            //then XITouchClass is never met, 
            //then cause m_touchDevices has not any touch device, 
            //then cause has_touch_without_mouse_emulation never be set in xiSelect2
            //then cause synthesizeMouseFromTouchEventsEnabled() is never met in QGuiApplicationPrivate::processTouchEvent
            //finally cause QT's SynthesizeMouseForUnhandledTouchEvents NEVER work
            //DHT, 20141022, currently we use single-touch screen in xf86-input-evdev-2.6.0(which input is single touch and output is mouse) and frontshell(qian-mian-ke) driver
