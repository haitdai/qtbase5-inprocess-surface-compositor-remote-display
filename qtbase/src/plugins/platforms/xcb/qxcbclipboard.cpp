    ...
    , m_incr_receive_time(0)
{
    ...
    if (connection()->hasXFixes()) {
        ...
        Q_XCB_CALL(xcb_xfixes_select_selection_input(xcb_connection(), m_owner, XCB_ATOM_PRIMARY, mask));
        Q_XCB_CALL(xcb_xfixes_select_selection_input(xcb_connection(), m_owner, atom(QXcbAtom::CLIPBOARD), mask));
    }
}
