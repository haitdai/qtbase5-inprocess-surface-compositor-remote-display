...
/*!
    FIXME 20150310!!!
    xserver depth == 24, image fomat == RGB32, 有alpha通道但没有alpha值, 
    但我们需要alpha 值，且把这个值传递给xrender
*/
static inline QImage::Format imageFormatForDepth(int depth)
{
    switch (depth) {
        case 32: return QImage::Format_ARGB32_Premultiplied;
        case 24: return QImage::Format_RGB32;                             //32bpp but 24bit DEPTH,  x8r8g8b8 in fact
...
#ifdef QT_MROFS
    , m_compositor(NULL)
    , m_pixmap_shadow(XCB_NONE)
    , m_pict_shadow(XCB_NONE)
    , m_pict(XCB_NONE)
    , m_gc(XCB_NONE)
#endif    
{
    ...
    if (window->type() != Qt::ForeignWindow)           //foreignwindow 表示外部创建的(如xserver)的window，qt无需再创建
        create();                                                          //xcb_create_window
    else
        m_window = window->winId();                        //xcb_window_t
}

/*!
    QXcbWindow 只属于xcb_screen_t.root 的窗口树，同时也属于某个QXcbScreen (默认全部创建在qt primary screen 上)
    ref. QWindow.fromWid, QWindow.QWindow, QWindow.setScreen(QWidget.create_sys), QGuiApplicationPrivate::screen_list, QXcbScreen.setVirtualSiblings
*/
void QXcbWindow::create()
{
    ...
    xcb_window_t xcb_parent_id = m_screen->root();                                         //TLW!!!
    if (parent()) {
        xcb_parent_id = static_cast<QXcbWindow *>(parent())->xcb_window();      //if has qt parent, use qt parent's xcb window as parent (xcb window)            NATIVE WIDGETS!!!
    ...
    int propertyCount = 0;
    properties[propertyCount++] = atom(QXcbAtom::WM_DELETE_WINDOW);     //期望被wm 管理关闭
    properties[propertyCount++] = atom(QXcbAtom::WM_TAKE_FOCUS);        //期望被wm 管理焦点(对于qt实际上是activated)
    properties[propertyCount++] = atom(QXcbAtom::_NET_WM_PING);         //期望被wm 测试是否阻塞

    m_usingSyncProtocol = m_screen->syncRequestSupported() && window()->surfaceType() != QSurface::OpenGLSurface;

    if (m_usingSyncProtocol)
        properties[propertyCount++] = atom(QXcbAtom::_NET_WM_SYNC_REQUEST); //期望和wm 同步repaint, ref. updateSyncRequestCounter
    ...
    Q_XCB_CALL(xcb_change_property(xcb_connection(),
                                   ...
                                   m_window,
                                   atom(QXcbAtom::WM_PROTOCOLS),        //window manager protocol
                                   XCB_ATOM_ATOM,
                                   32,
                                   propertyCount,
                                   properties));
    ...
    if (!wmClass.isEmpty()) {
        Q_XCB_CALL(xcb_change_property(xcb_connection(), XCB_PROP_MODE_REPLACE,
                                       m_window, atom(QXcbAtom::WM_CLASS),      //window manager class
...
void QXcbWindow::setCompositor(QMrOfsCompositor *compositor)
{
...
{
    QRect oldWmGeometry = windowToWmGeometry(geometry());
    QSize oldWmSz = QSize(oldWmGeometry.width(), oldWmGeometry.height());
...
#ifdef QT_MROFS
    QSize newWmSz = QSize(wmGeometry.width(), wmGeometry.height());
    if(m_compositor && newWmSz != oldWmSz) {
        m_compositor->setWindowGeometry(wmGeometry);
    }
#endif  // QT_MROFS
...
void QXcbWindow::setTransparentForMouseEvents(bool transparent)
{
    ...
    xcb_xfixes_set_window_shape_region(xcb_connection(), m_window, XCB_SHAPE_SK_INPUT, 0, 0, region);
...
/*!
    \note
    对于alien widgets, raise/lower 被qt内部处理了，这里只处理native window/TLW 的raise/lower
*/
...
/*!
    xserver 并没有做什么特别的处理(无需担心强制de-activated prev window)，而是直接以notify 的形式把_NET_ACTIVE_WINDOW 回给qt
    \ref  ProcSendEvent, WriteToClient, QXcbWindow::handleClientMessageEvent (case _NET_ACTIVE_WINDOW)
*/
...
void QXcbWindow::handleClientMessageEvent(const xcb_client_message_event_t *event)
{
    ...
    if (event->type == atom(QXcbAtom::WM_PROTOCOLS)) {              //window manager messages
        if (event->data.data32[0] == atom(QXcbAtom::WM_DELETE_WINDOW)) {        //close client
            QWindowSystemInterface::handleCloseEvent(window());
        } else if (event->data.data32[0] == atom(QXcbAtom::WM_TAKE_FOCUS)) {    // wm 设焦点到this
            connection()->setTime(event->data.data32[1]);
        } else if (event->data.data32[0] == atom(QXcbAtom::_NET_WM_PING)) {     // wm 测试this 是否阻塞
            ...
            xcb_flush(xcb_connection());
        } else if (event->data.data32[0] == atom(QXcbAtom::_NET_WM_SYNC_REQUEST)) { //wm 通知sync counter 更新
            connection()->setTime(event->data.data32[1]);
...
#ifndef QT_NO_DRAGANDDROP
    } else if (event->type == atom(QXcbAtom::XdndEnter)) {              //dnd
        connection()->drag()->handleEnter(window(), event);
    } else if (event->type == atom(QXcbAtom::XdndPosition)) {                       //dnd
        connection()->drag()->handlePosition(window(), event);
    } else if (event->type == atom(QXcbAtom::XdndLeave)) {                          //dnd
        connection()->drag()->handleLeave(window(), event);
    } else if (event->type == atom(QXcbAtom::XdndDrop)) {                           //dnd
        connection()->drag()->handleDrop(window(), event);
#endif
    } else if (event->type == atom(QXcbAtom::_XEMBED)) {                //类似OLE
        handleXEmbedMessage(event);
    } else if (event->type == atom(QXcbAtom::_NET_ACTIVE_WINDOW)) {     // !!! wm 通知this activated
        //屏蔽xcb focus in/out 后,  this(tlw) 只在map时通知active, 之后不再收到任何active/de-active，因此tlw可永远保持activated 状态
        //启动第二个qt时，虽然在xserver端第一个qt的tlw已被de-activated，但第一个qt本身仍保持activated状态，因此evdev的key消息可正常处理!!!
        connection()->setFocusWindow(this);
        QWindowSystemInterface::handleWindowActivated(window());        // 更新QGuiApplicationPrivate::focus_window
    } else if (event->type == atom(QXcbAtom::MANAGER)                   
               || event->type == atom(QXcbAtom::_NET_WM_STATE)          //window 状态(model, sticky, maximized, shaded, hidden, ...)
               || event->type == atom(QXcbAtom::WM_CHANGE_STATE)) {
...
/*!
    鼠标在多个tlw 之间的切换导致的tlw active/de-activate 的切换，由xserver和wm 负责，qt不处理。
    mrofs 下xserver 不处理鼠标，因此每个tlw的activated状态在map 时确定，之后不再变化。
    \ref.  QGuiApplicationPrivate::processMouseEvent
*/
...
/*!
    单向通知qt, 注意xcb focus event 和QFocusEvent 是2个东西，前者实际上是qt 的active event
*/
...
{
#ifdef QT_MROFS_INPUTMGR
    return QPlatformWindow::setKeyboardGrabEnabled(grab);       //evdev replace xi
#else    
    if (!grab) {
...
{
#ifdef QT_MROFS_INPUTMGR
    return QPlatformWindow::setMouseGrabEnabled(grab);       //evdev replace xi
#else   
    if (!grab) {
...
/*!
    if xcb cursor used, called by QXcbCursor::changeCursor
    if mrofs cursor used, called by nothing
    \ref QXcbCursor::changeCursor, QMrOfsCursor::changeCursor
*/
...
#ifdef QT_MROFS
xcb_render_picture_t QXcbWindow::shadowPicture()
{
    return m_pict_shadow;
}

xcb_render_picture_t QXcbWindow::picture()
{
    return m_pict;
}

/*!
    create shadow pixmap for this window
*/
xcb_pixmap_t QXcbWindow::createShadowPixmap(const QRect rect) 
{
    m_pixmap_shadow = xcb_generate_id(xcb_connection());
    xcb_generic_error_t *error = xcb_request_check(xcb_connection(), 
        xcb_create_pixmap_checked(xcb_connection(), 
        m_depth, m_pixmap_shadow, m_screen->root(), rect.width(), rect.height()));
    if (error) {
        qFatal("QXcbWindow::create, xcb_create_pixmap_checked failed: pixmap(%d), depth(%d), width(%d), height(%d)", 
            m_pixmap_shadow, m_depth, rect.width(), rect.height());
    }

    return m_pixmap_shadow;
}

void QXcbWindow::destroyShadowPixmap()
{
    if(XCB_NONE != m_pixmap_shadow) {
        xcb_free_pixmap(xcb_connection(), m_pixmap_shadow);
        m_pixmap_shadow = XCB_NONE;
    }
}

xcb_render_picture_t QXcbWindow::createShadowPicture()
{
    xcb_generic_error_t *error = NULL;

    //TODO: 如果边缘锯齿，打开PolyEdgeSmooth
    xcb_render_pictformat_t fmtId = pictFormatFromVisualId(xcb_connection(), m_visualId);       //使用 screen root visual
    m_pict_shadow = xcb_generate_id(xcb_connection());
    error = xcb_request_check(xcb_connection(), 
        xcb_render_create_picture_checked(xcb_connection(), m_pict_shadow, m_pixmap_shadow, fmtId, 0, 0));
    if (error) {
        qFatal("QXcbWindow::createShadowPicture, xcb_render_create_picture_checked failed: pict(0x%x), pixmap(0x%x), error_code(%d)", 
            m_pict_shadow, m_pixmap_shadow, error->error_code);
    }  
...
/*!
    create adaptor picture for xcb window(drawable)    
    \note 
    pixmap 的format 可在支持的format 列表中任意选取，但xcb window 的picture 必须和window 的visual format 兼容
*/
xcb_render_picture_t QXcbWindow::createPicture()
{
    xcb_generic_error_t *error = NULL;

    //TODO: 如果边缘锯齿，打开PolyEdgeSmooth
    xcb_render_pictformat_t fmtId = pictFormatFromVisualId(xcb_connection(), m_visualId);       //使用 screen root visual
    m_pict = xcb_generate_id(xcb_connection());
    error = xcb_request_check(xcb_connection(), xcb_render_create_picture_checked(xcb_connection(), m_pict, m_window, fmtId, 0, 0));
    if (error) {
        qFatal("QXcbWindow::createPicture, xcb_render_create_picture_checked failed: pict(0x%x), win(0x%x), error_code(%d)", 
            m_pict, m_window, error->error_code);
    }  
    
    return m_pict;
}

void QXcbWindow::destroyShadowPicture()
{
    xcb_generic_error_t *error = NULL;
    if (m_pict_shadow != XCB_NONE) {
        error = xcb_request_check(xcb_connection(), 
            xcb_render_free_picture_checked(xcb_connection(), m_pict_shadow));
        if (error) {
            qFatal("QXcbWindow::destroyShadowPicture, xcb_render_free_picture failed: pict(0x%x), error_code(%d)", 
                m_pict_shadow, error->error_code);
        }    
        m_pict_shadow = XCB_NONE;
    }
}
...
/*!
    以XCB_NONE(0) 下发会导致render 报错，因此判断非XCB_NONE 再free
    此picture 属于xcb window, 必须在xcb window destroyed 之前free 
*/
void QXcbWindow::destroyPicture()
{
    xcb_generic_error_t *error = NULL;
    if (m_pict != XCB_NONE) {
        error = xcb_request_check(xcb_connection(), 
            xcb_render_free_picture_checked(xcb_connection(), m_pict));
        if (error) {
            qFatal("QXcbWindow::destroyPicture, xcb_render_free_picture failed: pict(0x%x), error_code(%d)", 
                m_pict, error->error_code);
        }    
        m_pict = XCB_NONE;
    }
}

xcb_pixmap_t QXcbWindow::shadowPixmap()
{
    return m_pixmap_shadow;
}

xcb_gcontext_t QXcbWindow::gc()
{
    return m_gc;
}

xcb_gcontext_t QXcbWindow::createGC()
{
    xcb_generic_error_t *error = NULL;
    uint32_t gc_value_mask = XCB_GC_SUBWINDOW_MODE | XCB_GC_GRAPHICS_EXPOSURES;
    uint32_t gc_value_list[] = { XCB_SUBWINDOW_MODE_INCLUDE_INFERIORS, 0};
    m_gc = xcb_generate_id(xcb_connection());
    error = xcb_request_check(xcb_connection(), 
        xcb_create_gc_checked(xcb_connection(), m_gc, m_window, gc_value_mask, gc_value_list));
    if(error) {
        qFatal("QXcbWindow::createGC, xcb_create_gc_checked failed: gc(%d)", m_gc);
    }

    return m_gc;
}

void QXcbWindow::destroyGC()
{
    xcb_generic_error_t *error = NULL;
    if(XCB_NONE != m_gc) {
        error = xcb_request_check(xcb_connection(), 
            xcb_free_gc_checked(xcb_connection(), m_gc));
        if(error) {
            qFatal("QXcbWindow::destroyGC, xcb_free_gc_checked failed(%d)", m_gc);
        }
        m_gc = XCB_NONE;
    }
}
