...
//WATCHOUT: may cause OOM if uncomment this, ref. q_xcb_call_template and QXcbConnection::log
//#define Q_XCB_DEBUG_CALL
...
public:
    ...
    //DHT 这里可能有一个bug
    //QXcbScreen 以output 一一对应
    //xcb_screen_t 则可能包含多个output, m_primaryScreen 的语义是xcb_screen_t 的，即:0.1 表示m_primaryScreen == 1; :0.0 表示m_primaryScreen == 0
    //如果用QXcbConnection.screens.at(QXcbConnection.primaryScreen()) 获取 QXcbScreen，当一个xcb_screen_t 包含多个output 时，获得的QXcbScreen 是错误的
...
private:
    ...
    QXcbNativeInterface *m_nativeInterface;               // DHT  useful
...
/*!
    \note Q_XCB_NOOP does nothing(since xcb is asynchrous) whether Q_XCB_DEBUG defined or undefined.
    XSync is replaced by xcb_aux_sync in xcb by the same reason(i.e, API must wait for xserver's reply explicitly, the reply itself is meanless).
*/
#ifdef Q_XCB_DEBUG_CALL
template <typename cookie_t>
