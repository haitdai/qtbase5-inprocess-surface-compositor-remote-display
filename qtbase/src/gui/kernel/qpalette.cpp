...
public:
    ...
    QAtomicInt ref;
    QBrush br[QPalette::NColorGroups][QPalette::NColorRoles];     //  3*20
    int ser_no;
    int detach_no;
};
...
void QPalette::setBrush(ColorGroup cg, ColorRole cr, const QBrush &b)
{
    ...
    if(cg >= (int)NColorGroups) {
        if(cg == All) {
            for(int i = 0; i < (int)NColorGroups; i++)         //DHT20140928, set a column(ALL ColorGroups of specific ColorRole)
                d->br[i][cr] = b;
...
void QPalette::detach()
{
    if (d->ref.load() != 1) {
        ...
        if(!d->ref.deref())
            delete d;
        d = x;                  //DHT20140928, default QPalettePrivate::operator=
    }
    ++d->detach_no;
}
