    ...
    , QSurface(QSurface::Window)
{
    ...
    if (!d->screen)
        d->screen = QGuiApplication::primaryScreen();     //DHT 默认创建在PRIMARY SCREEN 上
    connect(d->screen, SIGNAL(destroyed(QObject*)), this, SLOT(screenDestroyed(QObject*)));
    QGuiApplicationPrivate::window_list.prepend(this);
}
...
void QWindow::setVisible(bool visible)
{
    ...
    //DHT  native window handle 在首次显示时才创建
    if (!d->platformWindow)
        create();
...
/*!
    ...
    情形1:  setVisible触发
    情形2: setScreen触发
    情形3, setParent触发(createWinId)
    情形4: setFlags触发
    情形5: AA_ImmediateWidgetCreation触发，除此之外都是按需触发
*/
void QWindow::create()
{
    Q_D(QWindow);
...
WId QWindow::winId() const
{
    ...
    //native window handle  在首次调用winId 时创建
    if(!d->platformWindow)
...
void QWindow::screenDestroyed(QObject *object)
{
    ...
    if (object == static_cast<QObject *>(d->screen)) {
        ...
        setScreen(0);       //DHT rebind this window to PRIMARY SCREEN
...
/*!
    ...
    注意这个是返回QWindow(非qt创建)；但所有由qt驱动生成的native window，都是返回QWidgetWindow(create_sys)
*/
QWindow *QWindow::fromWinId(WId id)
{
    ...
    window->create();      //创建真正的platformWindow
    return window;
}
