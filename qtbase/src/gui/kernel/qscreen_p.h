...
public:
        ...
        , orientationUpdateMask(0)
    {
        ...
        platformCompositor = NULL;
#endif		
    }
    
#ifdef QT_MROFS_LEGACY	
    QScreenPrivate(QPlatformScreen *screen, QPlatformCompositor *compositor)
        : platformScreen(screen), platformCompositor(compositor)
        , orientationUpdateMask(0)
    {
        orientation = screen->orientation();
        geometry = screen->geometry();
...
#ifdef QT_MROFS_LEGACY
    QPlatformCompositor *platformCompositor;               //DHT  screen own it's PRIVATE compositor
#endif    
