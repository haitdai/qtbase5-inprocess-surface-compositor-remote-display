...
public:
    ...
    public:
        ...
        QPointer<QWindow> window;               //DHT20141126, almost earilest receiver (in Qt subsystem) of all events delivered by QPA subsystem !!!
        bool nullWindow;
    ...
    class MouseCursorMoveEvent : public InputEvent {
    public:
        MouseCursorMoveEvent(QWindow * w, ulong time, const QPointF & local, const QPointF & global,
                   Qt::MouseButtons b, Qt::KeyboardModifiers mods)
            : InputEvent(w, time, MouseCursorMove, mods), localPos(local), globalPos(global), buttons(b) { }
        MouseCursorMoveEvent(QWindow * w, ulong time, EventType t, const QPointF & local, const QPointF & global,
                   Qt::MouseButtons b, Qt::KeyboardModifiers mods)
            : InputEvent(w, time, t, mods), localPos(local), globalPos(global), buttons(b) { }
        QPointF localPos;
        QPointF globalPos;
