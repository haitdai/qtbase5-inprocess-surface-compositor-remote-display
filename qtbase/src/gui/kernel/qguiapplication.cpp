...
static QMutex screenListMutex(QMutex::Recursive);
...
/*!
    ...
    such as key events.
    UPDATED in notification.
    \sa QApplication::activeWindow(), QApplication::focusWidget()
*/
...
void QGuiApplication::lockScreenList()
{
    screenListMutex.lock();
}

void QGuiApplication::unlockScreenList()
{
    screenListMutex.unlock();
}
...
static void init_platform(const QString &pluginArgument, const QString &platformPluginPath, int &argc, char **argv)
{
    ...
    QStringList arguments = pluginArgument.split(QLatin1Char(':'));           // DHT -platform xcb[[:<host>]:<major.minor>]...
    const QString name = arguments.takeFirst().toLower();                           // DHT name is xcb
...
{
    switch(e->type) {
    case QWindowSystemInterfacePrivate::MouseCursorMove:
        QGuiApplicationPrivate::processMouseCursorMoveEvent(static_cast<QWindowSystemInterfacePrivate::MouseCursorMoveEvent *>(e));    
        break;
...
/*!
    !!! inputmgr thread !!!
    localPoint ONLY, move ONLY, do NOT care button state, TLW(current screen) coordinate
    NOTE 1:
    CAUTION(MARKED WITH <<<): ui thread CAN add/removeOne screen_list, and CAN delete QXcbScreen, QScreen, QMrofsV2Cursor.
    ref. dtor of QPlatformScreen, QXcbConnection.updateScreens, QPlatformIntegration.screenAdded
    NOTE 2:
    ui can do read/write, inputmgr do read ONLY
*/
void QGuiApplicationPrivate::processMouseCursorMoveEvent(QWindowSystemInterfacePrivate::MouseCursorMoveEvent *e)
{
    QWindow *window = e->window.data();
    modifier_buttons = e->modifiers;
    
    QPointF localPoint = e->localPos;           // effective position, should be in TLW coordinate
    QPointF globalPoint = e->globalPos;

    QGuiApplication::lockScreenList();
    
    if (e->nullWindow || !window) {
        window = QGuiApplication::topLevelAt(globalPoint.toPoint());                 // <<<  1, find the TLW, main/sub screen's is different
        if(!window) {
            QGuiApplication::unlockScreenList();        
            return;
        }
        QPointF delta = globalPoint - globalPoint.toPoint();
        localPoint = window->mapFromGlobal(globalPoint.toPoint()) + delta;  // <<< 2, map to TLW coordinate
    }
    
    QMouseEvent ev(QEvent::MouseMove, localPoint, localPoint, globalPoint, Qt::NoButton, Qt::NoButton, e->modifiers);
    ev.setTimestamp(e->timestamp);

#ifndef QT_NO_CURSOR
    if (!e->synthetic) {
        if (const QScreen *screen = window->screen())                                   // <<< 3, window->screen() are always primary, ref ctor of QWindow
            if (QPlatformCursor *cursor = screen->handle()->cursor())               // <<< 4, cursor in primary screen since of 3
...
void QGuiApplicationPrivate::processMouseEvent(QWindowSystemInterfacePrivate::MouseEvent *e)
{
    ...
    // 如果input 没指定target , 则用tlw，注意这里分2种情况:
    // 后端为xcb 时，input 指定了target，因此直接使用指定的target
    // 后端为evdev 时，input 没指定target，因此投递给tlw
    if (e->nullWindow) {
    ...
    QGuiApplication::sendSpontaneousEvent(window, &ev);             // 鼠标点击默认获得焦点,  ref. QApplication::giveFocusAccordingToFocusPolicy
...
void QGuiApplicationPrivate::processKeyEvent(QWindowSystemInterfacePrivate::KeyEvent *e)
{
    ...
    if (e->nullWindow
        ...
        window = QGuiApplication::focusWindow();                // DHT  如果input 没指定target, 则用 focusWindow 
    }
...
void QGuiApplicationPrivate::processEnterEvent(QWindowSystemInterfacePrivate::EnterEvent *e)
{
    ...
    QCoreApplication::sendSpontaneousEvent(e->enter.data(), &event);        //QWidget::enterEvent
}
...
void QGuiApplicationPrivate::processLeaveEvent(QWindowSystemInterfacePrivate::LeaveEvent *e)
{
    ...
    QCoreApplication::sendSpontaneousEvent(e->leave.data(), &event);        //QWidget::leaveEvent
}
...
void QGuiApplicationPrivate::processActivatedEvent(QWindowSystemInterfacePrivate::ActivatedWindowEvent *e)
{
    ...
    QGuiApplicationPrivate::focus_window = newFocus;                // activated tlw
    if (!qApp)
        return;
...
/*!
    DHT 没人调, ref. handleWindowScreenChanged
*/
void QGuiApplicationPrivate::processWindowScreenChangedEvent(QWindowSystemInterfacePrivate::WindowScreenChangedEvent *wse)
{
    if (QWindow *window  = wse->window.data()) {
        if (QScreen *screen = wse->screen.data())
            window->d_func()->setScreen(screen, false /* NOT recreate */);
    ...
    for (int i = 0; i < e->points.count(); ++i) {
        ...
        case Qt::TouchPointPressed:
            ...
            if (!w) {
                ...
                if (!window)
                    window = QGuiApplication::topLevelAt(touchPoint.screenPos().toPoint());           //DHT20141126, deliver to top level WINDOW(not QWidget) first !!!
                if (!window)
                    continue;
                w = window;
            }
