...
static uint qt_gradient_pixel_fixed(const QGradientData *data, int fixed_pos)
{
    ...
    return data->colorTable[qt_gradient_clamp(data, ipos)];             //DHT20141114, ref QGradientCache::generateGradientColorTable
}
...
static void blend_color_rgb16(int count, const QSpan *spans, void *userData)
{
    ...
    if (mode == QPainter::CompositionMode_Source) {                                                        //DHT20141113, without alpha blending
        // inline for performance
        ...
        while (count--) {
            ...
            if (spans->coverage == 255) {
                ...
                //qWarning("!!!!!!!!!!!!!color(%x), c(%d,  %x), x(%d), y(%d), horiz pixels num(%d)", data->solid.color, c, c, spans->x, spans->y, spans->len);
            } else {
            //qWarning("!!!!!!!!!!!!!!alpha for RGB16");
                ushort color = BYTE_MUL_RGB16(c, spans->coverage);
                int ialpha = 255 - spans->coverage;
                const ushort *end = target + spans->len;
                while (target < end) {
                    *target = color + BYTE_MUL_RGB16(*target, ialpha);                                      //DHT20141113, this shows how to do alpha blending for RGB16
                    ++target;
                }
            }
            ++spans;
    ...
    if (mode == QPainter::CompositionMode_SourceOver) {                                                    //DHT20141113, with alpha blending
        while (count--) {
...
static void qt_gradient_quint16(int count, const QSpan *spans, void *userData)
{
    ...
    if (isVerticalGradient) {
        ...
        while (count--) {
            ...
            quint32 color = qt_gradient_pixel_fixed(&data->gradient, yinc * y + off);            //DHT20141113, calculating GRADIENT color in ARGB

            data->solid.color = color;
            blend_color_rgb16(1, spans, userData);                                                            //DHT20141113, fill/blend span with transformed color in RGB16
            ++spans;
        }
...
//DHT
//src * src_a/255 + dst * dst_a/255 => dst
//dst_a = map[i]
//src_a = 255 - map[i]
...
DrawHelper qDrawHelper[QImage::NImageFormats] =
{
    ...
    // Format_RGB32,
    {
        blend_color_argb,               // blendColor
        qt_gradient_quint32,            // blendGradient
        qt_bitmapblit_quint32,          // bitmapBlit
        qt_alphamapblit_quint32,        // alphamapBlit
        qt_alphargbblit_quint32,        // alphaRGBBlit
        qt_rectfill_argb32              // fillRect
    },
    // Format_ARGB32,
    {
        blend_color_generic,
    ...
    {
        blend_color_rgb16,
        qt_gradient_quint16,               //DHT20141113, blendGradient
...
#if defined(QT_COMPILER_SUPPORTS_NEON) && !defined(Q_OS_IOS) && (0 != 0)           // DHT  neon blending gets a little higher cpu comsuption than non-neon's under [full-screen parameter alarm situation]
    if (features & NEON) {
        qBlendFunctions[QImage::Format_RGB32][QImage::Format_RGB32] = qt_blend_rgb32_on_rgb32_neon;
        qBlendFunctions[QImage::Format_ARGB32_Premultiplied][QImage::Format_RGB32] = qt_blend_rgb32_on_rgb32_neon;

        // DHT [!!!BE CAREFUL!!!]    if using PVR2DMemAlloc, comment all neon functions
        //DHT default is        qt_blend_argb32_on_argb32
