...
//
//***********************************************************************************************************************
QMDrawCmdFileEngine::~QMDrawCmdFileEngine()
{ 	
	if( m_pPrintFileCreate) 
	{
		PrintInterfaceReleaseObject(m_pPrintFileCreate);
		m_pPrintFileCreate = NULL;
	}
}

QMDrawCmdFileEngine::QMDrawCmdFileEngine():QPaintEngine(QPaintEngine::AllFeatures), m_pPrintFileCreate(NULL), m_WHDpiRatio(1)
{
}

void QMDrawCmdFileEngine::SetJobId(QString jobId)
{
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetJobID( jobId.toStdString().c_str());
}

QString QMDrawCmdFileEngine::GetJobId()
{
	CHAR buf[256];
	memset(buf, 0, sizeof(buf) );

	if( m_pPrintFileCreate )
		m_pPrintFileCreate->GetJobID(buf, sizeof(buf) );  
	return buf;
}

void QMDrawCmdFileEngine::SetTotalPage(DWORD totalPage)
{	
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetTotalPage( totalPage );
}

DWORD QMDrawCmdFileEngine::GetTotalPage()
{
	if( m_pPrintFileCreate )
		return m_pPrintFileCreate->GetTotalPage();
	else
		return 0;
}

void QMDrawCmdFileEngine::SetCurrentPage(DWORD currentPage)
{
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetCurrentPage( currentPage );
}

DWORD QMDrawCmdFileEngine::GetCurrentPage()
{
	if( m_pPrintFileCreate )
		return m_pPrintFileCreate->GetCurrentPage();
	else
		return 0;
}

VOID QMDrawCmdFileEngine::SetRotate90(BOOL bRotate90)
{
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetRotate90(bRotate90);
}


BOOL QMDrawCmdFileEngine::GetRotate90()
{
	if( m_pPrintFileCreate )
		return m_pPrintFileCreate->GetRotate90();
	else
		return FALSE;
}

VOID QMDrawCmdFileEngine::SetShowSpeed(FLOAT speed)
{
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetShowSpeed( speed );
}

FLOAT QMDrawCmdFileEngine::GetShowSpeed()
{
	if( m_pPrintFileCreate )
		return m_pPrintFileCreate->GetShowSpeed();
	else
		return 0;
}

VOID QMDrawCmdFileEngine::SetFileType( UCHAR  fileType )
{
	if( m_pPrintFileCreate )
		m_pPrintFileCreate->SetFileType(fileType);
}

UCHAR QMDrawCmdFileEngine::GetFileType()
{
	if( m_pPrintFileCreate )
		return m_pPrintFileCreate->GetFileType();

	else
		return 0;
}

VOID QMDrawCmdFileEngine::SetShowArea( int width, int height )
{
	if( m_pPrintFileCreate )
	{
		m_pPrintFileCreate->SetShowAreaWidth(width * m_WHDpiRatio);
		m_pPrintFileCreate->SetShowAreaHeight(height);
	}
}

VOID QMDrawCmdFileEngine::SetShowAreaDpiWToH( int dpiWToH )
{
	m_WHDpiRatio = dpiWToH;
}

bool QMDrawCmdFileEngine::begin(QPaintDevice *pdev)
{
	m_pPrintFileCreate = PrintInterfaceCreateObject();
	m_pPrintFileCreate->Init();
	painter()->setBrush(QColor(Qt::white));
	return true;
}

MPrintFileInterface::CmdFileData * QMDrawCmdFileEngine::GetCmdFileData()
{
	if( NULL == m_pPrintFileCreate )
		return NULL;

	DWORD length = m_pPrintFileCreate->GetCmdFileLength();

	if( length < 0 )
		return NULL;

	MPrintFileInterface::CmdFileData * pResult = new MPrintFileInterface::CmdFileData;
	m_pPrintFileCreate->WriteToCmdFile( pResult->s);    
	m_pPrintFileCreate->Reset();
	return pResult;
}

bool QMDrawCmdFileEngine::newPage()
{
	if( NULL == m_pPrintFileCreate )
		return false;
...
void QMDrawCmdFileEngine::drawPoints(const QPointF *points, int pointCount)
{
	if( NULL == m_pPrintFileCreate )
		return;

	int color = painter()->pen().color().value();
	QTransform transform =state->transform();
	for( int i = 0; i < pointCount; ++i )
	{
		m_pPrintFileCreate->DrawPixel((points[i].toPoint().x() + transform.dx()) * m_WHDpiRatio, points[i].toPoint().y() + transform.dy(),  color );
	}
}

void QMDrawCmdFileEngine::drawLines(const QLineF *lines, int lineCount)
{
	if( NULL == m_pPrintFileCreate )
		return;

	int color = painter()->pen().color().value();
	Qt::PenStyle style = painter()->pen().style();
	QTransform transform =state->transform();
	for( int i = 0; i < lineCount; ++i )
	{
		if ((lines[i].p1().toPoint().y() >= 0) && (lines[i].p2().toPoint().y() >= 0))
		{
			m_pPrintFileCreate->DrawLine(
				(lines[i].p1().toPoint().x() + transform.dx()) * m_WHDpiRatio, 
				lines[i].p1().toPoint().y() + transform.dy(), 
				(lines[i].p2().toPoint().x() + transform.dx()) * m_WHDpiRatio, 
				lines[i].p2().toPoint().y() + transform.dy(),  
				(UCHAR)style, color );
		}
		else
		{
			QLine line = lines[i].toLine();
			if (line.p1().y() < 0)
			{
			...
			m_pPrintFileCreate->DrawLine(
				(line.p1().x() + transform.dx()) * m_WHDpiRatio, 
				line.p1().y() + transform.dy(), 
				(line.p2().x() + transform.dx()) * m_WHDpiRatio, 
				line.p2().y() + transform.dy(),  
				(UCHAR)style, color );
		}
...
void QMDrawCmdFileEngine::drawRects(const QRectF *rects, int rectCount)
{
	if( NULL == m_pPrintFileCreate )
		return;
	...
	int color = painter()->pen().color().value();
	QTransform transform =state->transform();
	QPoint offset = QPointF(transform.dx(), transform.dy()).toPoint();
	for( int i = 0; i < rectCount; ++i )
	{
		QPoint p1 = QPoint((rects[i].topLeft().toPoint().x()+ offset.x())*m_WHDpiRatio ,rects[i].topLeft().toPoint().y() + offset.y());
		QPoint p2 = QPoint((rects[i].bottomRight().toPoint().x() + offset.x())* m_WHDpiRatio,rects[i].bottomRight().toPoint().y() + offset.y()) ;
		m_pPrintFileCreate->DrawLine(p1.x(), p1.y(), p2.x(), p1.y(), 1, color );
		m_pPrintFileCreate->DrawLine(p1.x(), p1.y(), p1.x(), p2.y(), 1, color );
		m_pPrintFileCreate->DrawLine(p1.x(), p2.y() - 1, p2.x(), p2.y() - 1, 1, color );
		m_pPrintFileCreate->DrawLine(p2.x() - 1, p1.y(), p2.x() - 1, p2.y(), 1, color );
	}
}

void QMDrawCmdFileEngine::drawPolygon(const QPointF *points, int pointCount, PolygonDrawMode mode)
{
}

void QMDrawCmdFileEngine::drawPath (const QPainterPath & path)
{
}

void QMDrawCmdFileEngine::drawTextItem(const QPointF &p, const QTextItem &textItem)
{
	if( NULL == m_pPrintFileCreate )
		return;

	QString txt = textItem.text();
	QFont printFont = textItem.font();
	QFont font(printFont.family(), printFont.pointSize());
	QFontMetrics metrics(font);
	QColor color = painter()->pen().color();
	QBrush brush = painter()->brush();
	uchar grayColor = (color.red() * 11 + color.green() * 16 + color.blue() * 5) / 32;
	int width = 0, height = 0;
	QTransform transform =state->transform();
	int startx = (p.toPoint().x() + transform.dx()) * m_WHDpiRatio;
	int startY = p.toPoint().y() + transform.dy();
	
	char fontName[256];
	memset( fontName, 0, sizeof(fontName) );
	sprintf( fontName, "%s|%d", printFont.family().toStdString().c_str(), printFont.pointSize() );	//������Ҫע������ttf�ļ���ôת

	if (MPrintFileInterface::FILE_TYPE_PRINT == m_pPrintFileCreate->GetFileType())
	{
		for( int i = 0; i < txt.size(); ++i )
		{
			width = metrics.width(txt.at(i) );
			height = metrics.height();
			QImage img(width, height, QImage::Format_ARGB32); 
			img.fill( brush.color());
			QPainter painter (&img );
			painter.setFont(font);
			painter.setPen(color);
			painter.drawText(0, 0, width, height, Qt::AlignLeft, txt.at(i));
			painter.end();
			UCHAR * pImageData = NULL;
			QImage scaledImg;
			QImage cvtImg;
			//ֻ����������dpi
			if( (2 == m_WHDpiRatio) || (4 == m_WHDpiRatio) || (8 == m_WHDpiRatio ) )
			{
				scaledImg = img.scaled(width * m_WHDpiRatio, height);
				cvtImg = scaledImg.convertToFormat(QImage::Format_Indexed8);
				pImageData = (UCHAR *)cvtImg.bits();
				width = width * m_WHDpiRatio;
			}
			else
			{
				//img.save("D:\\r.bmp");
				cvtImg = img.convertToFormat(QImage::Format_Indexed8);
				pImageData = (UCHAR *)cvtImg.bits();

				//cvtImg.save("D:\\t.bmp");

			}
			m_pPrintFileCreate->DrawChar(txt.at(i).unicode(), (UINT16)startx, (UINT16)startY, (UINT16)width, (UINT16)height, (CHAR*)pImageData, fontName, grayColor);
			startx += width;
		}
	}
	else if (MPrintFileInterface::FILE_TYPE_RECORD == m_pPrintFileCreate->GetFileType())
	{
		for( int i = 0; i < txt.size(); ++i )
		{
			...
			width = metrics.width(txt.at(i) );
			height = metrics.height();
			//[0,1,2,3,4,5,6,7]8���㱣�浽byte�е�˳����H->L
			...
			{
				scaledImg = img.scaled(width * m_WHDpiRatio, height);
				pImageData = (UCHAR *)scaledImg.bits();
				width = width * m_WHDpiRatio;
			}
			else
				pImageData = (UCHAR *)img.bits();
	...
	//	{
	//		scaledImg = img.scaled(width * m_WHDpiRatio, height);
	//		pImageData = (CHAR *)scaledImg.bits();
	//		width = width * m_WHDpiRatio;
	//	}
	//	else
	//		pImageData = (CHAR *)img.bits();
	//	
	//	//img.save("d:\\ttt.bmp");
	//	
	//	m_pPrintFileCreate->DrawChar(txt.at(i).unicode(), (UINT16)startx, (UINT16)startY, (UINT16)width, (UINT16)height, pImageData, fontName, grayColor);
	//	startx += width;
	//}
	
}

void QMDrawCmdFileEngine::drawPixmap (const QRectF & rectangle, const QPixmap & pixmap, const QRectF & sr)
{	
	//pixmap.save("d:\\form.bmp");
	drawImage(rectangle, pixmap.toImage(), sr, Qt::AutoColor);
}
//#pragma pack(1)
...
//#pragma pack(0)
void QMDrawCmdFileEngine::drawImage(const QRectF &r, const QImage &pm, const QRectF &sr,
                   Qt::ImageConversionFlags flags)
{
	//pm.save("D:\\widget.bmp");
	if( NULL == m_pPrintFileCreate )
		return;
	QTransform transform =state->transform();
	QPoint offset = QPointF(transform.dx(), transform.dy()).toPoint();
	QRect drawArea = r.toRect();
	QPoint start((drawArea.topLeft().x() + offset.x()) * m_WHDpiRatio, drawArea.topLeft().y() + offset.y());
	//drawArea.translate(offset);
	QImage convertImage;
	const QImage * dstRgbImgPtr = &pm;

	if(QImage::Format_RGB888 != pm.format() )
	{
		convertImage = pm.convertToFormat(QImage::Format_RGB888);
		dstRgbImgPtr = &convertImage;
	}
	...
	if( (2 == m_WHDpiRatio) || (4 == m_WHDpiRatio) || (8 == m_WHDpiRatio ) )
	{
		dstScaledImg = dstRgbImgPtr->scaled(dstRgbImgPtr->width() * m_WHDpiRatio, dstRgbImgPtr->height());
		pImageData = (CHAR *)dstScaledImg.bits();
		size = dstScaledImg.size();
		//dstScaledImg.save("d:\\123.bmp");
	}
	else
	{
		pImageData = (CHAR *)dstRgbImgPtr->bits();
		size = dstRgbImgPtr->size();
	}
		
	DWORD pictureId = (((DWORD)pm.cacheKey()) & 0x00FFFFFF);
	m_pPrintFileCreate->DrawPicture(pictureId, start.x(),  start.y(), size.width(),  size.height(), pImageData );

	//DWORD srcPerLineBytes = (drawArea.width() * 24 + 31) / 32 * 4;
	//if( drawArea.width() > 500 )	//for test
	//{
	//	BITMAPFILEHEADER fileHeader;
	//	fileHeader.bfType = 0x4D42; //BM
	//	fileHeader.bfSize = srcPerLineBytes * drawArea.height() + 54;
	...
	//	infoHeader.biWidth = drawArea.width();
	//	infoHeader.biHeight = drawArea.height();
	//	infoHeader.biPlanes = 0;
	//	infoHeader.biBitCount = 24;
	//	infoHeader.biCompression = 0;
	//	infoHeader.biSizeImage = srcPerLineBytes * drawArea.height();
	...
	//	FILE *fp = fopen("test1.bmp", "wb");
	//	fwrite((char *)&fileHeader, sizeof(BITMAPFILEHEADER), 1, fp);
	//	fwrite((char *)&infoHeader, sizeof(BITMAPINFOHEADER), 1, fp);
	//	fwrite(pImageData, infoHeader.biSizeImage, 1, fp);
	//	fclose(fp);
	//}
}

void QMDrawCmdFileEngine::drawTiledPixmap (const QRectF & rectangle, const QPixmap & pixmap, const QPointF & point)
{
}

void QMDrawCmdFileEngine::updateState(const QPaintEngineState &state)
{
}

int QMDrawCmdFileEngine::metric(QPaintDevice::PaintDeviceMetric metricType) const
{
	return 0;
}

QPaintEngine::Type QMDrawCmdFileEngine::type() const
{
	return QPaintEngine::MCmdDataFile;
}
 
void QMDrawCmdFileEngine::setPen()
{
}

void QMDrawCmdFileEngine::setBrush()
{
}

void QMDrawCmdFileEngine::setupGraphicsState(QPaintEngine::DirtyFlags flags)
{
}
