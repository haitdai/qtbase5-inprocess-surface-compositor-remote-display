...
// QSpanBuffer = blend(op) + QSpanData(pixels, as output) + clip(rect)
class QSpanBuffer {
public:
...
private:
    void flushSpans()
    {
        m_blend(m_spanCount, m_spans, m_data);      // qt_blend_argb32_on_argb32
        m_spanCount = 0;
    }
...
public:
    bool antialiased;
    bool legacyRounding;
    ProcessSpans blend;     // take charge of pixel generating
    void *data;             // QSpanData*, output
    QRect clipRect;

    QScanConverter scanConverter;   // take charge of beziers curves and straight lines
};
...
void QScanConverter::end()
{
    ...
    if (m_lines.size() <= 32) {
        ...
        for (int chunkTop = m_top; chunkTop <= m_bottom; chunkTop += CHUNK_SIZE) {
            ...
            emitSpans(chunkTop);        // blend to pixels
        }
    }
...
//DHT20141113, Fix Point(26.6): 2^6 == 64
...
void QRasterizer::rasterizeLine(const QPointF &a, const QPointF &b, qreal width, bool squareCap)
{
    ...
    QPointF pb = b;
    //DHT
    //qWarning("!!!!!!!!!!!!!!pa(%f, %f), pb(%f, %f), squareCap(%d)", pa.x(), pa.y(), pb.x(), pb.y(), squareCap);
    //END
    if (squareCap) {
        QPointF delta = pb - pa;
        pa -= (0.5f * width) * delta;
    ...
    //DHT 20141113, flush all lefted spans in destructor of QSpanBuffer
    //qWarning("!!!!!!!!!!!!!!pa(%f, %f), pb(%f, %f)", pa.x(), pa.y(), pb.x(), pb.y());
    //END
    ...
    if (q26Dot6Compare(pa.y(), pb.y())) {
        ...
        const qreal y = pa.y();
        const qreal dy = width * dx;              //DHT, 20141113, width actually is SLOPE
    ...
    //DHT
    //qWarning("!!!!!!!!!!!!!!pa(%f, %f), pb(%f, %f), width(%f)", pa.x(), pa.y(), pb.x(), pb.y(), width);
    //END
    ...
    //DHT
    //qWarning("!!!!!!!!!!!!!!left(%f), right(%f)", left, right);
    //END
        ...
        if (d->antialiased) {
            ...
            Q16Dot16 coverage[3];          //DHT20141113, coverage 表示直线在1 个像素点上的覆盖百分比，左中右，中间的始终为255(全覆盖)
            int x[3];
            int len[3];

    //DHT
    //qWarning("!!!!!!!!!!!!!!iLeft(%d), iRight(%d), leftWidth(%d), rightWidth(%d)", iLeft, iRight, leftWidth, rightWidth);
    //END
            ...
            if (iLeft == iRight) {
                ...
                coverage[0] = leftWidth * 255;                        //DHT20141113, coverage: [0, 255], 表示span 内的填充权重
                x[0] = iLeft;
                len[0] = 1;                                                       //DHT20141113, 左边若有则长度始终为1
                if (leftWidth == Q16Dot16Factor) {                    //DHT20141113, 左边无小数
                    len[0] = iRight - iLeft;
                } else if (iRight - iLeft > 1) {
                    coverage[1] = IntToQ16Dot16(255);          //中间全覆盖
                    x[1] = iLeft + 1;
                    len[1] = iRight - iLeft - 1;
                    ++n;      
                }
                if (rightWidth) {                                                  //DHT20141113, 右边有小数
                    coverage[n] = rightWidth * 255;
                    x[n] = iRight;
                    len[n] = 1;           //DHT20141113, 右边若有则长度始终为1
                    ++n;
                }
            }
            ...
            //DHT, try to find out why QPainter(QRect, ...) is much faster than QPainter(QRectF, ...)
            //qWarning("!!!!!!!!!!!!!!iTopFP(%d), iBottomFP(%d), n(%d)", iTopFP, iBottomFP, n);
            //END
            
            for (Q16Dot16 yFP = iTopFP; yFP <= iBottomFP; yFP += Q16Dot16Factor) {
                const Q16Dot16 rowHeight = qMin(yFP + Q16Dot16Factor, yPb) -qMax(yFP, yPa);    //DHT20141113, 大多数时候都是  rowHeight == Q16Dot16Factor
                const int y = Q16Dot16ToInt(yFP);
                if (y > d->clipRect.bottom())
                    break;
                for (int i = 0; i < n; ++i) {                                                                                      //DHT20141113,处理左中右，各占用1个span
        ...
        QPointF delta = pb - pa;
        delta *= 0.5f * width;                                       //DHT20141113  步长 , 斜率越小步长越小
        ...
        //DHT 20141113, FP == Fixed Point ???, 所有的FP 都是在原值基础上乘以 65535 得到的
