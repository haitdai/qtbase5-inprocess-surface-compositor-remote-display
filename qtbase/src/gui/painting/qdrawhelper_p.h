...
// QSpanData = compositemode + pixels + drawhelper + clip + matrix
struct QSpanData
{
    QSpanData() : tempImage(0) {}
    ~QSpanData() { delete tempImage; }

    QRasterBuffer *rasterBuffer;                    // pixels
    ProcessSpans blend;
...
{
    if (ipos < 0 || ipos >= GRADIENT_STOPTABLE_SIZE) {             //1024
...
inline ushort qConvertRgb32To16(uint c)
{
    ...
    int b1 = ((c) >> 10) & 0x003f;
    int b2 = b1 & 0x3e;
    if (qAbs(r /31.0 - b1*1.0/63.0) - qAbs(r/31.0 - b2*1.0/63.0) > 0.000001) {
        qWarning("!!!!!!!!!! use bit 0");
   return (((c) >> 3) & 0x001f)
       | (((c) >> 5) & 0x07c0)
       | (((c) >> 8) & 0xf800);
...
//DHT20141118, 16*16 = 256 degree grayscale (M8)
...
const uchar qt_dither_threshold_r[8*8] = {
    1, 7, 3, 5, 0, 8, 2, 6,
    7, 1, 5, 3, 8, 0, 6, 2,
    3, 5, 0, 8, 2, 6, 1, 7,
    5, 3, 8, 0, 6, 2, 7, 1,
...
const uchar qt_dither_threshold_g[8*8] = {
    1, 3, 2, 2, 3, 1, 2, 2,
    2, 2, 0, 4, 2, 2, 4, 0,
    3, 1, 2, 2, 1, 3, 2, 2,
    2, 2, 4, 0, 2, 2, 0, 4,
...
const uchar qt_dither_threshold_b[8*8] = {
    5, 3, 8, 0, 6, 2, 7, 1,
    3, 5, 0, 8, 2, 6, 1, 7,
    8, 0, 6, 2, 7, 1, 5, 3,
    0, 8, 2, 6, 1, 7, 3, 5,
