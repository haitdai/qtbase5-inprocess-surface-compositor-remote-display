...
bool QPainter::begin(QPaintDevice *pd)
{
    ...
    if (d->extended)
        d->extended->setState(d->state);                // DHT,  QRasterPaintEngine(QPaintEngineEx) share state with QPainter
    else
        d->engine->state = d->state;
...
//DHT - screen orientation switch supported via qpainter transform. obsolute ...
#if 0
    QScreen *pscrn = QGuiApplication::primaryScreen();
    Qt::ScreenOrientation  orientation = pscrn->orientation();
    qreal w = pscrn->geometry().width();
    qreal h = pscrn->geometry().height();
    
    //dubhe has only 1 screen, default is portrait
    if (Qt::PrimaryOrientation ==  orientation)
    {
        orientation = pscrn->primaryOrientation();   
        //should be portrait, ALWAYS.
        Q_ASSERT(Qt::PortraitOrientation == orientation);
    }

    if (Qt::PortraitOrientation ==  orientation) 
    {
        //do nothing
    }
    else if(Qt::LandscapeOrientation ==  orientation)
    {
        d->state->redirectionMatrix.translate(-w/2, -h/2);
        d->state->redirectionMatrix.rotate(90);
        d->state->redirectionMatrix.translate(w/2, h/2); 
    }
    else if(Qt::InvertedPortraitOrientation ==  orientation) 
    {
        d->state->redirectionMatrix.translate(-w/2, -h/2);
        d->state->redirectionMatrix.rotate(180);
        d->state->redirectionMatrix.translate(w/2, h/2);     
    }
    else 
    {
        d->state->redirectionMatrix.translate(-w/2, -h/2);
        d->state->redirectionMatrix.rotate(270);
        d->state->redirectionMatrix.translate(w/2, h/2);     
    }
#endif
//END DHT
