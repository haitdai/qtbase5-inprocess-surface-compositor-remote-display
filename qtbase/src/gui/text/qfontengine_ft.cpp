...
//DHT 将glyph (outline -> path) 光栅化到 QImage(alphaMap)
//返回值是个只有alpha 值的Indexed8 的QImage
QImage QFontEngineFT::alphaMapForGlyph(glyph_t g, QFixed subPixelPosition)
{
    ...
    QImage img(glyph->width, glyph->height, antialias ? QImage::Format_Indexed8 : QImage::Format_Mono);      //<<< DHT  注意Indexed8，这里是有alpha没有rgb
    if (antialias) {
    ...
    if (glyph->width) {
        for (int y = 0; y < glyph->height; ++y)
            memcpy(img.scanLine(y), &glyph->data[y * pitch], pitch);     //DHT 拷贝所有像素的alpha 
    }
    unlockFace();
