...
#ifdef QT_PRINTFILE    
	else if(format == QPrinter::MindrayPrintFormat)
		outputFormat = QPrinter::MindrayPrintFormat;
#endif
...
#ifdef QT_PRINTFILE    
    else if(outputFormat == QPrinter::MindrayPrintFormat)
    {
    	 QMDrawCmdPrintEngine * pDrawCmdPrintEnginePtr = new QMDrawCmdPrintEngine();
		 paintEngine = pDrawCmdPrintEnginePtr;
		 printEngine = pDrawCmdPrintEnginePtr;
    }
#endif    
   else {
...
void QPrinter::setOutputFileName(const QString &fileName)
{
    ...
    if (!fi.suffix().compare(QLatin1String("pdf"), Qt::CaseInsensitive))
        setOutputFormat(QPrinter::PdfFormat);
#ifdef QT_PRINTFILE        
   else if(!fi.suffix().compare(QLatin1String("mpf"), Qt::CaseInsensitive))
		setOutputFormat(QPrinter::MindrayPrintFormat);
   else if(!fi.suffix().compare(QLatin1String("mrf"), Qt::CaseInsensitive))
		setOutputFormat(QPrinter::MindrayPrintFormat);
#endif		
