                                                            ...
                                                            QEvent *event)
{
    ...
    case QEvent::TouchEnd: {
        if (q->state() != Qt::NoGesture) {
            if (ev->touchPoints().size() == 1) {  //YY
                QTouchEvent::TouchPoint p1 = ev->touchPoints().at(0);
                //QTouchEvent::TouchPoint p2 = ev->touchPoints().at(1);  YY
                d->lastOffset = d->offset;
                d->offset =
                        QPointF(p1.pos().x() - p1.startPos().x(),// + p2.pos().x() - p2.startPos().x(),  YY
                              p1.pos().y() - p1.startPos().y()); // + p2.pos().y() - p2.startPos().y()) / 2;  YY
            }
                                                              ...
                                                              QEvent *event)
{
    ...
    case QEvent::TouchUpdate: {
        if (!d->started)
            result = QGestureRecognizer::CancelGesture;
        else if (ev->touchPoints().size() == 1) {  //YY
            QTouchEvent::TouchPoint p1 = ev->touchPoints().at(0);
            //QTouchEvent::TouchPoint p2 = ev->touchPoints().at(1);  YY
            //QTouchEvent::TouchPoint p3 = ev->touchPoints().at(2);  YY

            if (d->lastPositions[0].isNull()) {
                d->lastPositions[0] = p1.startScreenPos().toPoint();
                //d->lastPositions[1] = p2.startScreenPos().toPoint();  YY
                //d->lastPositions[2] = p3.startScreenPos().toPoint();  YY
            }
            d->hotSpot = p1.screenPos();
            d->isHotSpotSet = true;

            int xDistance = /*(*/p1.screenPos().x() - d->lastPositions[0].x();/* +  YY
                             p2.screenPos().x() - d->lastPositions[1].x() +  YY
                             p3.screenPos().x() - d->lastPositions[2].x()) / 3;*/  //YY
            int yDistance = /*(*/p1.screenPos().y() - d->lastPositions[0].y();/* +  YY
                             p2.screenPos().y() - d->lastPositions[1].y() +  YY
                             p3.screenPos().y() - d->lastPositions[2].y()) / 3;*/  //YY
