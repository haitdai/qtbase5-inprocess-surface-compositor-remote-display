...
/*!
    ...
    UPDATED in setFocusWidget(which called by QWidget::setFocus, QApplication::setActiveWindow)
...
/*!
    ...
    UPDATED IN setActiveWindow
    \sa QWidget::setFocus(), QWidget::hasFocus(), focusWidget()
    \sa QGuiApplication::focusWindow()
    \sa focusWidget()
*/
...
bool QApplication::notify(QObject *receiver, QEvent *e)
{
    ...
    // DHT 20151030 
    // the Qt gesture guys mean to handle gesture state change on-the-fly, but who forget the following limitation: 
    // QApplication.notify could run in every QThread only if which has a eventloop but UI thread!
    // if e->type() is not an UI-exclusive event(UI-exclusive events includes Paint, KeyPress, ...), 
    // race-condition on [d->gestureManager] is established between this thread and UI thread.
    if (d->gestureManager) {
