...
/*!
    DHT20141104, 创建 parentWidget 的所有子孙中为Native 的
*/
...
void QWidgetPrivate::create_sys(WId window, bool initializeWindow, bool destroyOldWindow)
{
    ...
    win->create();         //createPlatformWindow -> new QXcbWindow, use qt's parent's native window as this parent(native window)
    ...
    if (!store) {
        if (win && q->windowType() != Qt::Desktop) {
            if (q->isTopLevel())
                q->setBackingStore(new QBackingStore(win)); //为TLW 创建backingstore
        } else {
            q->setAttribute(Qt::WA_PaintOnScreen, true);       //desktop is always paint on screen
        }
    }

    setWindowModified_helper();
    //DHT 20141104, 从 QPlatformWindow 获取 winid( ref. QFbWindow::winId)
    WId id = win->winId();       //QXcbWindow.winId
...
void QWidgetPrivate::setParent_sys(QWidget *newparent, Qt::WindowFlags f)
{
    ...
    if (parent != newparent) {
        ...
        //DHT, 20141104, 如果是Native，则调用 QPlatformWindow::setParent，对于mrofs/linuxfb，不支持(没有实现)。
        //对于 windows/xcb, 支持(ref. QXcbWindow.setParent)。
        //对于非Native(Alien)，不会执行。
        if (q->windowHandle()) {
...
void QWidgetPrivate::setGeometry_sys(int x, int y, int w, int h, bool isMove)
{
    ...
    if (q->isVisible()) {
        if (!q->testAttribute(Qt::WA_DontShowOnScreen) && !q->testAttribute(Qt::WA_OutsideWSRange)) {
            if (q->windowHandle()) {
                if (q->isWindow()) {
                    q->windowHandle()->setGeometry(q->geometry());          // QXcbWindow.setGeometry
                } else {
                ...
                if (bs && bs->store) {
                    if (isResize)
                        bs->store->resize(r.size());                                         // QXcbBackingstore.resize
                }
...
/*!
    DHT TLW 或native window 都有platform window，但只有TLW 有backingstore
*/
void QWidgetPrivate::createTLSysExtra()
{
    Q_Q(QWidget);
    if (!extra->topextra->window && (q->testAttribute(Qt::WA_NativeWindow) || q->isWindow())) {  //条件: native widget 或 TLW
        extra->topextra->window = new QWidgetWindow(q);                       // <<< create platform window: 注意这里只是创建了QWindow, 并没有立即创建native window handle(显示时才真正创建)
