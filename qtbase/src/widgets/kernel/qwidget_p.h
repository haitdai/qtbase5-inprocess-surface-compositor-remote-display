...
struct QTLWExtra {
    ...
    QWidgetBackingStoreTracker backingStoreTracker;       //DHT  注意QExtra/QTLWExtra 每个nativewindow 都有一个，但backingStoreTracker.data 只有TLW 才有!!!  ref. QWidget.create()
    QBackingStore *backingStore;                                       //DHT backingstoretracker.data
    ...
    bool wasMaximized;
#endif
    QWidgetWindow *window;          //<<< ref. QWidgetPrivate::createTLSysExtra,   native window 都有
...
struct QWExtra {
    ...
    QTLWExtra *topextra;                         // only useful for TLWs               DHT( !NULL for native widgets, NULL for alien widgets, ref. createTLSysExtra)
...
public:
    ...
    QWidget *focus_child;
    QLayout *layout;
    QRegion *needsFlush;        // DHT: native child's coordinate
