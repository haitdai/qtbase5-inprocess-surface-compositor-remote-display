    ...
    if (!project->values("QMAKE_APP_FLAG").isEmpty() || project->first("TEMPLATE") == "aux") {
        ...
        if(do_incremental) {
            ...
            if (project->first("TEMPLATE") != "aux") {
                ...
                if ( !project->isActiveConfig("use_link_arg_file") )
                {
                ...
                {
                    /*
                    @echo $(LFLAGS) -o $(TARGET)  > linkarg.tmp
                    @-for /R ./ARMLinux/tempobj_unx/ %%i in (*.obj) do echo %%i>> linkarg.tmp
                    @echo $(OBJCOMP) $(LIBS)  >> linkarg.tmp
                    @repl_cr linkarg.tmp link.arg
                    $(LINK) @link.arg
                    */
                    t << "@echo $(LFLAGS) " <<  var("QMAKE_LINK_O_FLAG") << "$(TARGET) > linkarg.tmp\n\t";
                    t << "@-for /R ./" << var("OBJECTS_DIR") << " %%i in (*.obj) do echo %%i>> linkarg.tmp\n\t";
                    t << "@echo $(OBJCOMP) $(LIBS)  >> linkarg.tmp\n\t";
                    t << "@repl_cr linkarg.tmp link.arg\n\t";
                    t << "$(LINK) @link.arg";
                }
