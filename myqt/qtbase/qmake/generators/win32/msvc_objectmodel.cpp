...
void VCProjectWriter::outputFilter(VCProject &project, XmlOutput &xml, const QString &filtername)
{
    ...
    for (int i = 0; i < project.SingleProjects.count(); ++i) {
        ...
        QString skipPath = project.SingleProjects.at(0).FilterSkipRelatePath;
        for (int x = 0; x < filter.Files.count(); ++x)
        {
            const VCFilterFile &file = filter.Files.at(x);
            QString filepath = file.file;
            if ( !skipPath.isEmpty() && filepath.startsWith(skipPath, Qt::CaseInsensitive) )
            {
                filepath.remove( 0, skipPath.length() );
            }
            root->addElement( filepath, file );
        }
