...
QMrOfsWaveform::QMrOfsWaveform(QMrOfsWaveview* wv, QAbstractMrOfs::Format fmt, const QRect& rc) 
    : format0(fmt), wid0(-1), geometry0(rc), dirty(QRegion()), effective_clip(QRegion()), effective_dirty(QRegion()), view(wv), pvr2d_image(NULL)
{
...
{
    QWindow *win = view->containerWidget()->window()->windowHandle();
    QMrOfsScreen *scrn = static_cast<QMrOfsScreen *>(win->screen()->handle());

    qDebug("QMrOfsWaveform::allocateData: create waveform image");
    this->pvr2d_image = new QMrOfsShmImage(scrn, size_in_pixel, depthFromMainFormat(fmt), fmt); 
...
{
    delete this->pvr2d_image;
    this->pvr2d_image = NULL;
}
...
{  
#if defined(DEBUG_GPU_BLT) && defined(QMROFS_MEM_ALLOCATED_CMEM)
    (reinterpret_cast<QMrOfsShmImage*>(pvr2d_image))->wb(dirty);
#endif
}
...
{
    return this->pvr2d_image->image()->size();
}
...
vpt0(QRect()), wfs(QList<QMrOfsWaveform*>()), vid0(-1), container_widget(container), controller0(wc), m_isIView(isIView)
{
    ...
    QWindow *win = container_widget->window()->windowHandle();
}
