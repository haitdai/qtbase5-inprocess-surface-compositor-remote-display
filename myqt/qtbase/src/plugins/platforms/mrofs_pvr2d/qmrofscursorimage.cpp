...
/*!
    ...
    永不析构
*/
QMrOfsCursorImage::QMrOfsCursorImage(QMrOfsScreen *screen, const uchar *data, 
    const uchar *mask, int width, int height, int hotX, int hotY) : pvr2DImage(NULL)
{ 
    this->screen = screen;                                    //screen OK
    set(data, mask, width, height, hotX, hotY);
    if(data) {
        createCursorPVR2D(width, height);
    }
}
...
QMrOfsShmImage *QMrOfsCursorImage::picture()
{
    return this->pvr2DImage;
}
...
/*!
    创建qImage 的镜像pvr2DImage, 并把qImage 的内容同步进去
    使用镜像的原因是保持cursorimage 和qImage 相关的接口不变
*/
void QMrOfsCursorImage::createCursorPVR2D(int width, int height)
{
    qDebug("QMrOfsCursorImage::createCursorPVR2D: create cursor picture, width(%d), height(%d)", width, height);
    pvr2DImage = new QMrOfsShmImage(screen, QSize(width, height), 
        depthFromMainFormat(QMROFS_FORMAT_DEFAULT), QMROFS_FORMAT_DEFAULT);

    // sync to pvr2d image
    QPainter blitter(this->pvr2DImage->image());
    blitter.setCompositionMode(QPainter::CompositionMode_Source);
    blitter.drawImage(QPoint(0, 0), this->qImage);
}
    ...
    switch (id) {
        ...
        case Qt::ArrowCursor:
            systemCursorTable[Qt::ArrowCursor] =
                new QMrOfsCursorImage(screen, cur_arrow_bits, mcur_arrow_bits, 16, 16, 0, 0);
            break;

        case Qt::UpArrowCursor:
            systemCursorTable[Qt::UpArrowCursor] =
                new QMrOfsCursorImage(screen, cur_up_arrow_bits, mcur_up_arrow_bits, 16, 16, 7, 0);
            break;

        case Qt::CrossCursor:
            systemCursorTable[Qt::CrossCursor] =
                new QMrOfsCursorImage(screen, cur_cross_bits, mcur_cross_bits, 16, 16, 7, 7);
            break;

        case Qt::IBeamCursor:
            systemCursorTable[Qt::IBeamCursor] =
                new QMrOfsCursorImage(screen, cur_ibeam_bits, mcur_ibeam_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeVerCursor:
            systemCursorTable[Qt::SizeVerCursor] =
                new QMrOfsCursorImage(screen, cur_ver_bits, mcur_ver_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeHorCursor:
            systemCursorTable[Qt::SizeHorCursor] =
                new QMrOfsCursorImage(screen, cur_hor_bits, mcur_hor_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeBDiagCursor:
            systemCursorTable[Qt::SizeBDiagCursor] =
                new QMrOfsCursorImage(screen, cur_bdiag_bits, mcur_bdiag_bits, 16, 16, 7, 7);
            break;

        case Qt::SizeFDiagCursor:
            systemCursorTable[Qt::SizeFDiagCursor] =
                new QMrOfsCursorImage(screen, cur_fdiag_bits, mcur_fdiag_bits, 16, 16, 7, 7);
            break;

        case Qt::BlankCursor:
            systemCursorTable[Qt::BlankCursor] =
                new QMrOfsCursorImage(screen, 0, 0, 0, 0, 0, 0);
            break;

        // 20x20 cursors
        case Qt::ForbiddenCursor:
            systemCursorTable[Qt::ForbiddenCursor] =
                new QMrOfsCursorImage(screen, forbidden_bits, forbiddenm_bits, 20, 20, 10, 10);
            break;

        // 32x32 cursors
        case Qt::WaitCursor:
            systemCursorTable[Qt::WaitCursor] =
                new QMrOfsCursorImage(screen, wait_data_bits, wait_mask_bits, 32, 32, 15, 15);
            break;

        case Qt::SplitVCursor:
            systemCursorTable[Qt::SplitVCursor] =
                new QMrOfsCursorImage(screen, vsplit_bits, vsplitm_bits, 32, 32, 15, 15);
            break;

        case Qt::SplitHCursor:
            systemCursorTable[Qt::SplitHCursor] =
                new QMrOfsCursorImage(screen, hsplit_bits, hsplitm_bits, 32, 32, 15, 15);
            break;

        case Qt::SizeAllCursor:
            systemCursorTable[Qt::SizeAllCursor] =
                new QMrOfsCursorImage(screen, size_all_data_bits, size_all_mask_bits, 32, 32, 15, 15);
            break;

        case Qt::PointingHandCursor:
            systemCursorTable[Qt::PointingHandCursor] =
                new QMrOfsCursorImage(screen, phand_bits, phandm_bits, 32, 32, 0, 0);
            break;

        case Qt::WhatsThisCursor:
            systemCursorTable[Qt::WhatsThisCursor] =
                new QMrOfsCursorImage(screen, whatsthis_bits, whatsthism_bits, 32, 32, 0, 0);
            break;
        case Qt::BusyCursor:
            systemCursorTable[Qt::BusyCursor] =
                new QMrOfsCursorImage(screen, busy_bits, busym_bits, 32, 32, 0, 0);
            break;

        case Qt::OpenHandCursor:
            systemCursorTable[Qt::OpenHandCursor] =
                new QMrOfsCursorImage(screen, openhand_bits, openhandm_bits, 16, 16, 8, 8);
            break;
        case Qt::ClosedHandCursor:
            systemCursorTable[Qt::ClosedHandCursor] =
                new QMrOfsCursorImage(screen, closedhand_bits, closedhandm_bits, 16, 16, 8, 8);
            break;
        default:
...
void QMrOfsCursorImage::set(Qt::CursorShape id)
{
    ...
    this->qImage = cursor->qImage;
    this->pvr2DImage = cursor->pvr2DImage;
    this->hot = cursor->hot;
}
...
/*!
    data -> curImage(cursor_data)
    data: indexd8, 只取3色
    mask: index8
    curImage(cursor_data): QMROFS_FORMAT_DEFAULT
*/
                    ...
                    int width, int height, int hx, int hy)
{
    ...
    this->qImage = QImage(width, height, qtFormatFromMainFormat(QMROFS_FORMAT_DEFAULT));
    
    if (!width || !height || !data || !mask || this->qImage.isNull())
        return;

    int bytesPerLine = (width + 7) / 8;
    unsigned int p = 0;                                 //QMROFS_FORMAT_DEFAULT
    int d, m;

    int x = -1, w = 0;

    unsigned int *cursor_data = reinterpret_cast<unsigned int*>(this->qImage.bits());
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < bytesPerLine; j++, data++, mask++)
        {
            for (int b = 0; b < 8 && j*8+b < width; b++)// bitwise
            {
                d = *data & (1 << b);
                m = *mask & (1 << b);
                if (d && m) p = PREMUL(0xff000000);             //opaque black
                else if (!d && m) p = PREMUL(0xffffffff);       //opaque white
                else p = PREMUL(0x00000000);                    //transparent black
                cursor_data[j*8+b] = p;
