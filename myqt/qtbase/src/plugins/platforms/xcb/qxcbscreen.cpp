...
/*
    如果是多outputs(ref. BD 多屏), 那么一个qt 程序会产生多个QXcbScreen 实例, 每个对应一个output
*/
QXcbScreen::QXcbScreen(QXcbConnection *connection, xcb_screen_t *scr,
    ...
    , m_xSettings(0)
{
    //DHT register screen configuration changed notification
    //DHT xcb_randr_select_input 的第3个参数是mask, ref randr.h
    //true == 1 == RRScreenChangeNotifyMask                                                                                 [randr v1.0]
    //实际上还有其他的mask:
    //RRCrtcChangeNotifyMask | RROutputChangeNotifyMask | RROutputPropertyNotifyMask                 [randr1.2]
    //RRProviderChangeNotifyMask | RRProviderPropertyNotifyMask | RRResourceChangeNotifyMask     [randr1.4]
    ...
    //DHT 本 client 的首个显示用窗口，是屏幕root window的儿子，相当于顶层窗口
    m_clientLeader = xcb_generate_id(xcb_connection());
    Q_XCB_CALL2(xcb_create_window(xcb_connection(),
                                  XCB_COPY_FROM_PARENT,
                                  m_clientLeader,
                                  screen()->root,           //DHT parent
                                  0, 0, 1, 1,               //DHT leader window's geometry, 几乎不可见
                                  0,
...
#ifdef  QT_MROFS
    //since all widgets are created on primary qt screen, cursors in non-primary qt screens have no chance to show
    m_cursor = new QMrOfsCursor(this);
#else
...
QWindow *QXcbScreen::topLevelAt(const QPoint &p) const
{
    ...
    do {
        ...
        //DHT xcb_window_t to QPlatformWindow(QXcbWindow)
        QPlatformWindow *platformWindow = connection()->platformWindowFromId(child);
        if (platformWindow)
            return platformWindow->window();  //DHT QPlatformWindow to QWindow
    } while (parent != child);
...
QPlatformCursor *QXcbScreen::cursor() const         // QPlatformScreen::cursor
{
    return m_cursor;
}
...
#ifdef Q_XCB_DEBUG
                qDebug("QXcbScreen::updateGeometry: m_geometry(%d, %d, %d, %d), %s", 
                    m_geometry.x(), m_geometry.y(), m_geometry.width(), m_geometry.height(), qPrintable(m_outputName));
#endif            
...
//DHT  xcb_window_t -(copy)> xcb_pixmap_t -(copy)> memory QImage -> QRasterPlatformPixmap(memor QImage)
