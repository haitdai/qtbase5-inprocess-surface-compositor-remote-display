...
//创建QXcbScreen(QXcbConnection) ，由xcb event 触发
//ref. updateScreens
    ...
    int screenNumber, xcb_screen_t* xcbScreen, xcb_randr_get_output_info_reply_t *output)
{
...
#ifdef Q_XCB_DEBUG
    qDebug("[findOrCreateScreen] Screen output name: %s, screenNumber(%d) ", qPrintable(name), screenNumber);
#endif
    } else {
...
#ifdef Q_XCB_DEBUG
    qDebug("[findOrCreateScreen] found existed in m_screens: %s, screenNumber(%d), QXcbScreen(%p)", qPrintable(name), screenNumber, scr);
#endif            
            return scr;
        }
    }
    ...
    newScreens << ret;
#ifdef Q_XCB_DEBUG
    qDebug("[findOrCreateScreen] newScreens: %s, screenNumber(%d), QXcbScreen(%p)", qPrintable(name), screenNumber, ret);
#endif    
    return ret;
}
...
void QXcbConnection::updateScreens()
{
    ...
    int xcbScreenNumber = 0;    // screen number in the xcb sense (DHT xcb_screen_t)
    QSet<QXcbScreen *> activeScreens;
    QList<QXcbScreen *> newScreens;
    QXcbScreen* primaryScreen = NULL;
    while (it.rem) {
        // Each "screen" in xcb terminology is a virtual desktop,                                        (DHT 可能包含多个 outputs)
        // potentially a collection of separate juxtaposed monitors.
        // But we want a separate QScreen for each output (e.g. DVI-I-1, VGA-1, etc.)      (DHT QScreen 只有1个output)
        ...
        if (has_randr_extension) {
            ...
            xcb_randr_get_output_primary_cookie_t primaryCookie =
                xcb_randr_get_output_primary(xcb_connection(), xcbScreen->root);                       //DHT  xcb_screen 可能有多个outputs, primary output 由xrandr 指定, primary output  的坐标总是较小
            xcb_randr_get_screen_resources_current_cookie_t resourcesCookie =
                xcb_randr_get_screen_resources_current(xcb_connection(), xcbScreen->root);
            //用前面返回的序列号查询真正的返回值
            xcb_randr_get_output_primary_reply_t *primary =
                    xcb_randr_get_output_primary_reply(xcb_connection(), primaryCookie, &error);  //DHT  xrandr primary output as qt's primary screen
            if (!primary || error) {
                qWarning("QXcbConnection: Failed to get the primary output of the screen");            //DHT 虽然xserver 容忍没有primary output, 但qt 不容忍
                free(error);
            } else {
                ...
                if (!resources || error) {
                    ...
                    xcb_randr_output_t *outputs = xcb_randr_get_screen_resources_current_outputs(resources);     //DHT 当前xcb_screen 有几个outputs
                    ...
                    //DHT 找当前xcb_screen root window 所在的outputs ，注意1个xcb_screen_t可能 对应多个QXcbScreen
                        QXcbScreen *screen = findOrCreateScreen(newScreens, xcbScreenNumber, xcbScreen, output);
                        siblings << screen;
                        activeScreens << screen;
                        ++screenNumber;
#ifdef Q_XCB_DEBUG
                                qDebug("siblings added: %s, screenNumber(%d), xcbScreenNumber(%d), QXcbScreen(%p)", 
                                    qPrintable(outputName), screenNumber, xcbScreenNumber, screen);
#endif                        
                        if (!primaryScreen && primary) {
                            //DHT 注意 m_primaryScreen 可能不为  0 ，例如当 DISPLAY =:0.1 时，m_primaryScreen == 1
                            if (m_primaryScreen == xcbScreenNumber && (primary->output == XCB_NONE || outputs[i] == primary->output)) {
                                primaryScreen = screen;        
                                siblings.prepend(siblings.takeLast());  //DHT 若是primaryScreen 则从尾部取下放到首部
#ifdef Q_XCB_DEBUG
    ...
    QGuiApplication::lockScreenList();
    ...
    foreach (QXcbScreen* screen, newScreens) {
        if (screen == primaryScreen)
            m_screens.prepend(screen);       //DHT primaryScreen 始终在首位
        else
            m_screens.append(screen);        //DHT 其余的在后面
    }
    
    // Now that they are in the right order, emit the added signals for new screens only
    foreach (QXcbScreen* screen, m_screens) {
        if (newScreens.contains(screen)) {
            ((QXcbIntegration*)QGuiApplicationPrivate::platformIntegration())->screenAdded(screen);          //创建qt screen (QScreen)            
#ifdef Q_XCB_DEBUG
                qDebug("QGuiApplicationPrivate::screen_list added: QXcbScreen(%p)", screen);
#endif            
        }
    }
    QGuiApplication::unlockScreenList();    
}
    ...
    , m_systemTrayTracker(0)
{
    ...
    if (dpy) {
        m_primaryScreen = DefaultScreen(dpy);                                 //DHT :0.N, N 为screen number
...
#ifdef XCB_USE_EGL
    EGLDisplay eglDisplay = eglGetDisplay(dpy);  //DHT,  a xcb connection actually,  ref  mesa eglapi.c/platform_x11.c/native_x11.c.
...
#endif
#ifdef XCB_USE_RENDER
        &xcb_render_id,           //DHT extern declared in render.h
#endif
...
#ifdef QT_MROFS
    checkExtensions(m_connection);             //fatal if failed
#endif  //QT_MROFS
...
qWarning("defined XCB_USE_XINPUT2");
    initializeXInput2();
#endif
    initializeXShape();
    initializeXKB();
    ...
    //DHT event listener window (class input only)
    ...
    //TODO: DHT create a compositor for this connection
}
...
{
    //DHT listener is QXcbWindow
    QXcbWindowEventListener *listener = m_mapper.value(id, 0);
    if (listener)
        return listener->toWindow();   // DHT type casting
    return 0;
}
...
/*!
    keyboard event 都是给了 focus window, 必须保证每个qt 实例的focus window 不被xserver 强制互斥(可共存)
*/
...
/*!
    用evdev 替换xi event 后，只需保证每个qt实例的唯一的native window 始终为activated 状态即可保证每个实例
    都能收到键盘消息，内部widget之间的焦点控制由qt完成。
    actiaved 状态被map event 控制。
*/
void QXcbConnection::handleXcbEvent(xcb_generic_event_t *event)
{
    ...
    if (!handled) {
        ...
        case XCB_EXPOSE:            //paint
            HANDLE_PLATFORM_WINDOW_EVENT(xcb_expose_event_t, window, handleExposeEvent);
        case XCB_BUTTON_PRESS:
#ifdef QT_MROFS_INPUTMGR
            handled = true;
            break;                  //replaced by evdev
#else
#ifdef QT_NO_XKB
...
//#endif  //QT_MROFS_INPUTMGR          
    ...
    if (!handled) {
        if (response_type == xfixes_first_event + XCB_XFIXES_SELECTION_NOTIFY) {
            ...
            updateScreens();                                // 更新 qt primary screen, virtual siblings
            xcb_randr_screen_change_notify_event_t *change_event = (xcb_randr_screen_change_notify_event_t *)event;
            foreach (QXcbScreen *s, m_screens) {            // 更新每个 qt screen 的尺寸
...
{
    if (m_xcb_poll_for_queued_event) {
        // m_connection is in main(UI) thread; QXcbEventReader.eventPending is emited QXcbEventReander thread
...
void QXcbEventReader::registerForEvents()
{
    ...
    connect(dispatcher, SIGNAL(aboutToBlock()), m_connection, SLOT(processXcbEvents()));           //m_connection 在界面线程创建,  ref. QXcbIntegration ctor
...
void QXcbEventReader::run()
{
    ...
    while (m_connection && (event = xcb_wait_for_event(m_connection->xcb_connection()))) {
        m_mutex.lock();
        addEvent(event);
        while (m_connection && (event = m_xcb_poll_for_queued_event(m_connection->xcb_connection())))              //把reply/event 一把全读上来
            addEvent(event);
        m_mutex.unlock();
        emit eventPending();
    }
...
//[ui thread], ref. registerForEvents
void QXcbConnection::processXcbEvents()
{
    ...
    for(int i = 0; i < eventqueue->size(); ++i) {
        ...
        if (!response_type) {
            handleXcbError((xcb_generic_error_t *)event);  // 所有xserver 错误都以event 形式上报(异步上报)，这里优先处理错误
        } else {
#ifndef QT_MROFS_INPUTMGR        
