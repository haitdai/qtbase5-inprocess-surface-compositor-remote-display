...
/*!
    DHT add.
    mark pixmap dirty outside, so scaled pixmap must be synced
*/
void QLabel::markPixmapDirty()
{
    Q_D(QLabel);
    d->pixmap_dirty = true;
}
...
void QLabel::paintEvent(QPaintEvent *)
{
    ...
    if (d->pixmap && !d->pixmap->isNull()) {
        QPixmap pix;
        if (d->scaledcontents) {
            if (!d->scaledpixmap || d->scaledpixmap->size() != cr.size() || d->pixmap_dirty) {
                if(d->pixmap_dirty) {
