...
#endif
    if (desktopWidget) {        //if we have desktopWidget, all widget sit screen where desktopWidget sits
...
/*!
    ...
    注意: parentWidget == NULL 或type == Qt::Window 或type ==Qt::SubWindow 将导致widget 为创建TLW/BS；
    不满足上述条件的native window 也不是TLW, 而是和它的祖先中最近的满足上述条件的widget 共享1个TLW/BS。
*/
...
void QWidget::create(WId window, bool initializeWindow, bool destroyOldWindow)
{
    ...
    d->create_sys(window, initializeWindow, destroyOldWindow);                      //<<<<<<< DHT  创建native window(QWidgeWindow)

    // a real toplevel window needs a backing store, 注意不是所有native window 都有bs，只有window type == Qt::Window 的才有!!!
    if (isWindow() && windowType() != Qt::Desktop) {
        d->topData()->backingStoreTracker.destroy();
        if (hasBackingStoreSupport())
            d->topData()->backingStoreTracker.create(this);                                       //<<<<<<< DHT 创建platform backingstore(仅TLW)
    }
...
#ifdef QT_MROFS
QRect QWidgetPrivate::clipRectNoGE() const
{
    Q_Q(const QWidget);
    const QWidget * w = q;
    if (!w->isVisible())
...
/*!
    \internal
    DHT - paintOnScreen 的局限性:
    以 glwidget 为例说明, glwidget 是直接由opengl (esv2) 库绘制在native window(eglWindowSurface/wglWindow)上的,
    它没有backingstore 的后端.
    当它上方再出现一个qt 的 alien widget (和它为兄弟关系或父子关系)时, 由于egl/wgl 无法感知到这个alien widget,
    因此会覆盖掉它们重叠的区域(正确的逻辑应该是避开或按照正确的z序合成这个区域).
    解决方法有2个:
    方法1: 在paintOnScreen 的在自绘时剪掉或正确合成这个区域.
    方法2: 让glwidget 绘制到fbo(带裁减), 再将backingstore 和fbo都变成texture 后合成.
    注意: 
    1, 通过将上方alien widget 变为native widget(如winId)不能解决这个问题, 因为native widget 还是由qt绘制, 并在backingstore.flush
    的时候同步到tlw, 仍然会被paintOnScreen 的glwidget 覆盖一部分.
    2, mrofs 和方法2类似, 但fbo 的对等物(QWaveData) 由QPainter 更新而非gl.
    3, 方法1需要自己实现, 方法2见qt5.5 对QOpenGLWidget 的实现.
*/
...
void QWidgetPrivate::updateIsOpaque()
{
    ...
    if (q->autoFillBackground()) {
        ...
        if (autoFillBrush.style() != Qt::NoBrush && autoFillBrush.isOpaque()) {         //DHT 如果autoFillBackground 使用的brush(alpha = 0xFF) 是opauqe, 则widget 为opaque
            setOpaque(true);
            return;
        }
    }
...
void QWidgetPrivate::setOpaque(bool opaque)
{
    if (isOpaque != opaque) {
        isOpaque = opaque;
        updateIsTranslucent();   //DHT 20141104, 本 Window 背景是否为透明
    }
}
...
void QWidgetPrivate::paintBackground(QPainter *painter, const QRegion &rgn, int flags) const
{
    ...
    if ((flags & DrawAsRoot) && !(q->autoFillBackground() && autoFillBrush.isOpaque())) {      //第1次画bg, DHT 非opaque widget(ref. updateIsOpaque)
        const QBrush bg = q->palette().brush(QPalette::Window);                         //半透明背景的背景，最下一层
        if (!(flags & DontSetCompositionMode)) {
            //copy alpha straight in
            QPainter::CompositionMode oldMode = painter->compositionMode();
            painter->setCompositionMode(QPainter::CompositionMode_Source);      //mode = Source 的目标是为了拷贝alpha 
            fillRegion(painter, rgn, bg);                                                               //拷贝，包括alpha
            painter->setCompositionMode(oldMode);                                              //回复老的mode, 可能是Over
        } else {
            fillRegion(painter, rgn, bg);
        }
    }

    if (q->autoFillBackground())                                                                                           //第2次画bg, 使用第1次填入的alpha(半透明背景效果)
...
/*!
    ...
    注意: 调用QWidget::winId 或QWidget::setAttribute(Qt::WA_NativeWindow) 将导致widget 及其祖先全部变native；
    Qt::WA_DontCreateNativeAncestors 可控制只让widget 变native，而祖先不必。
*/
...
/*
    \note DHT 20141104,
    如果this 是Native 的，则直接创建(QPlatformWindow)
    否则，递归创建一个子树，子树的根为距离this最近的Native，且根到this之间的所有节点(this 的所有祖先)都为native
    注意区别Qt::WA_NativeWindow 和Qt::Window/Qt::SubWindow
*/
void QWidgetPrivate::createWinId(WId winid)
{
    ...
    if (!q->testAttribute(Qt::WA_WState_Created) || (forceNativeWindow && !q->internalWinId())) {
        if (!q->isWindow()) {              //DHT 需要winid 但又不是Qt::Window, 则只能是native widget
            QWidget *parent = q->parentWidget();
            QWidgetPrivate *pd = parent->d_func();
            if (forceNativeWindow && !q->testAttribute(Qt::WA_DontCreateNativeAncestors))
                parent->setAttribute(Qt::WA_NativeWindow);
            if (!parent->internalWinId()) {
                pd->createWinId();          //DHT, 20141104, create parent first
            }

            //DHT 20141104, create childrens, except for self(as for self, raise it)
            for (int i = 0; i < pd->children.size(); ++i) {
                QWidget *w = qobject_cast<QWidget *>(pd->children.at(i));
                if (w && !w->isWindow() && (!w->testAttribute(Qt::WA_WState_Created)
                                            || (!w->internalWinId() && w->testAttribute(Qt::WA_NativeWindow)))) {    //没wid 但有WA_NativeWindow说明native window尚未创建
                    if (w!=q) {
                        w->create();              //注意有Qt::WA_NativeWindow 没有Qt::Window/SubWindow
                    } else {
                        w->create(winid);
            ...
            q->create();             //DHT, 20141104, -> QWidget::create
        }
    }
}
...
/*!
    \note DHT, rgn is in widget itself coordinate
    \note DHT, draw all non-opaques in widget tree, from root to leaf, drawWidget <=> paintSiblingsRecursive;
    if some node is opaque, do it without recursion
    \see QWidgetBackingStore::sync()
*/
                                ...
                                QPainter *sharedPainter, QWidgetBackingStore *backingStore)
{
    ...
    const bool alsoOnScreen = flags & DrawPaintOnScreen;        // DHT, repaint_sys(paint on screen)有, sync(no paint on screen)无
    const bool recursive = flags & DrawRecursive;               // DHT, repaint_sys(paint on screen) 无
    ...
    if (!(flags & DontSubtractOpaqueChildren))                  //DHT, subtract opaque region from toBePainted
        subtractOpaqueChildren(toBePainted, q->rect());

    if (!toBePainted.isEmpty()) {                               // DHT, 处理background
        if (!onScreen || alsoOnScreen) {                        // DHT, 不是paint on screen, 或者是paint on screen 同时需要draw
...
#endif
            QPaintEngine *paintEngine = pdev->paintEngine();    //DHT for paint on screen(alsoOnScreen), always return 0
            if (paintEngine) {
                setRedirected(pdev, -offset);                   // DHT redirect widget(this) to TLW, QPainter always do it's job in TLW
    ...
    //DHT, 处理foreground(do children's subtree) - 如果paint on screen 则不会执行, ref. DrawRecursive
                                            ...
                                            , QPainter *sharedPainter, QWidgetBackingStore *backingStore)
{
    ...
    do {
        ...
        if (x && !(exludeOpaqueChildren && x->d_func()->isOpaque) && !x->isHidden() && !x->isWindow()
            ...
            if (dirtyBoundingRect) {
                boundingRect = rgn.boundingRect();          // DHT faster region ops but slower drawWidget(much more widgets to visit)
    ...
    QWidgetPrivate *wd = w->d_func();
    const QPoint widgetPos(w->data->crect.topLeft());       // w 在它的parent 中的坐标
    ...
    if (w->updatesEnabled()
        ...
        wRegion.translate(-widgetPos);                      // 从parent 坐标转换为 w 的坐标, rgn 为parent 坐标
        if (hasMask)
...
*/
//DHT20141125, seems for QGraphicsView ONLY
...
//    qWarning("QWidget::update[QRect]");
    if (testAttribute(Qt::WA_WState_InPaintEvent)) {
//            qWarning("QWidget::update: WA_WState_InPaintEvent");
...
void QWidget::update(const QRegion &rgn)
{
    ...
    if (r.isEmpty())
        return;
//    qWarning("QWidget::update[QRegion]");
    if (testAttribute(Qt::WA_WState_InPaintEvent)) {
//                    qWarning("QWidget::update: WA_WState_InPaintEvent");
        QApplication::postEvent(this, new QUpdateLaterEvent(r));
        return;
...
void QWidget::setAttribute(Qt::WidgetAttribute attribute, bool on)
{
    ...
    case Qt::WA_NativeWindow: {
        d->createTLExtra();
        if (on)
            d->createTLSysExtra();              //DHT  create for native window
#ifndef QT_NO_IM
