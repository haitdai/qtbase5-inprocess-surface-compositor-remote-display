 ...
 * \a region is the region to be updated in \a TLW coordinates.
 */
                            ...
                            QWidget *tlw, const QPoint &tlwOffset)
{
    ...
    //tlwOffset:  TLW 相对screen(linuxfb, mrofs, eglfs, ...)或windowsurface(windows, xcb, ...) 的offset, 一般为(0, 0)
    if (widget != tlw)      //非tlw 的其他native widget(如,调用了winId), 它们在qt 窗口树中存在父子关系
        backingStore->flush(region, widget->windowHandle(), tlwOffset + widget->mapTo(tlw, QPoint()));        //非tlw native, region 用native widget coordinate, ref. markDirtyOnScreen
    else
        backingStore->flush(region, widget->windowHandle(), tlwOffset);                                        //tlw alien, region 用TLW coordinate
}
                                     ...
                                     BeginPaintInfo *returnInfo, bool toCleanIsInTopLevelCoordinates)
{
    ...
    dirtyOnScreen += toClean;                       // DHT 20160413, dirtyOnScreen 包括2部分: update的, 以及手工调用markDirtyOnScreen加入的(比如滚动)
...
//DHT20141121, SHOULD WE try move flush() before endPaint to make beginPaint/endPaint lock takes effect? ref QMrOfsBackingStore.endPaint
        ...
        BeginPaintInfo *beginPaintInfo)
{
    ...
    backingStore->endPaint2();      // for unlocking in endPaint
}
...
QRegion QWidgetBackingStore::dirtyRegion(QWidget *widget) const
{
    ...
    if (dirtyOnScreenWidgets) { // Only in use with native child widgets
        for (int i = 0; i < dirtyOnScreenWidgets->size(); ++i) {
            ...
            r += wd->needsFlush->translated(w->mapTo(tlw, QPoint()));       // DHT needsFlush is in native child's coordinate, so translate to TLW's
        }
    }
...
/*!
    ...
    If invalidateBuffer is true, all widgets intersecting with the region will be dirty.                     <<<<<<<<<<<DHT 20141027
                                    ...
                                    bool invalidateBuffer)
{
    ...
    //DHT
    //qWarning("markDirty_QRegion >>>> rgn.translated(offset)(%d, %d, %d, %d)", 
//        rgn.translated(offset).boundingRect().x(), rgn.translated(offset).boundingRect().y(), rgn.translated(offset).boundingRect().width(), rgn.translated(offset).boundingRect().height());
    //END
        if (!eventAlreadyPosted || updateImmediately)
            sendUpdateRequest(tlw, updateImmediately);
        return;
                                    ...
                                    bool invalidateBuffer)
{
    ...
    //DHT
    //qWarning("markDirty_QRect >>>> translatedRect(%d, %d, %d, %d)", translatedRect.x(), translatedRect.y(), translatedRect.width(), translatedRect.height());
    //END
    ...
    if (widget->d_func()->inDirtyList) {
        if (!qt_region_strictContains(widget->d_func()->dirty, widgetRect))
            widget->d_func()->dirty += widgetRect;      // DHT 20160413, 已经在dirty widget list 中, 更新dirty region
    } else {
        addDirtyWidget(widget, rect);                   // DHT 20160413, 不在dirty widget list 中, 加入
    }
...
/*!
    ...
    Paint on screen widgets are ignored.                           <<<<<DHT
    topLevelOffset means offset of the widget to TLW         <<<<<DHT
*/
void QWidgetBackingStore::markDirtyOnScreen(const QRegion &region, QWidget *widget, const QPoint &topLevelOffset)
{
    ...
    if (!widget->internalWinId() && !widget->isWindow()) {
        ...
        if (nativeParent == tlw) {
            if (!widget->testAttribute(Qt::WA_WState_InPaintEvent))
                dirtyOnScreen += region.translated(topLevelOffset);       //TLW coordinate
            return;
        }
        ...
        *nativeParentPrivate->needsFlush += region.translated(nativeParentOffset);         // DHT  needsFlush is in native child's coordinate
...
void QWidgetPrivate::moveRect(const QRect &rect, int dx, int dy)    // rect is the widget rect in old position
{
    Q_Q(QWidget);
    ...
    QRect clipR(pd->clipRect());                                        // parent clipped by all grandfathers
    ...
    bool accelerateMove = accelEnv && isOpaque                            //DHT 20141230  isOpaque 通过 Qt::WA_OpaquePaintEvent || Qt::WA_PaintOnScreen 设置
    ...
    if (!accelerateMove) {                                                                //DHT 20141230 只有isOpaque 的widget才能加速滚动
        ...
        //DHT20141125
        //qWarning("!accelerateMove   >>>>>>>>>>>>>>>>>>");
    } else {

    /*! DHT 20141125
        qWarning("accelerateMove   >>>toplevelOffset(%d,%d), dx(%d), dy(%d), rect(%d,%d,%d,%d)", 
            toplevelOffset.x(), toplevelOffset.y(), dx, dy, rect.x(), rect.y(), rect.width(), rect.height());
*/
        QWidgetBackingStore *wbs = x->backingStoreTracker.data();
        QRegion childExpose(newRect & clipR);
        
        if (sourceRect.isValid() && wbs->bltRect(sourceRect, dx, dy, pw))           //DHT 20160413 scrolled in parent,  可被 QPlatformBackingStore.scroll 加速
            childExpose -= destRect;

        if (!pw->updatesEnabled())                                                  //DHT 20141230, 滚动时关闭 parent的update，可以加快滚动速度 (跳过 markDirty)
            return;
        ...
        if (!parentExpose.isEmpty()) {
            wbs->markDirty(parentExpose, pw);                                       // DHT 20160413, 这部分需要update 和sync
            pd->isMoved = true;
        }
        ...
        if (childUpdatesEnabled) {
            ...
            needsFlush += destRect;
            wbs->markDirtyOnScreen(needsFlush, pw, toplevelOffset);                 // DHT 20160413, 这部分无需update，但需要sync
        }

/*! DHT20141125
        qWarning("accelerateMove   >>>newRect(%d,%d,%d,%d), clipR(%d,%d,%d,%d)", 
            newRect.x(), newRect.y(), newRect.width(), newRect.height(),
            clipR.x(), clipR.y(), clipR.width(), clipR.height());
        qWarning("accelerateMove   >>>sourceRect(%d,%d,%d,%d), destRect(%d,%d,%d,%d)",         
            sourceRect.x(), sourceRect.y(), sourceRect.width(), sourceRect.height(),
            destRect.x(), destRect.y(), destRect.width(), destRect.height());           
        qWarning("accelerateMove   >>>childExpose(%d,%d,%d,%d), parentExpose(%d,%d,%d,%d)",         
            childExpose.boundingRect().x(), childExpose.boundingRect().y(), childExpose.boundingRect().width(), childExpose.boundingRect().height(),
            parentExpose.boundingRect().x(), parentExpose.boundingRect().y(), parentExpose.boundingRect().width(), parentExpose.boundingRect().height());             
            */
    }
}
...
void QWidgetPrivate::scrollRect(const QRect &rect, int dx, int dy)
{
    ...
    //DHT 加速条件(一般不满足), 
    //opaque 时，由于下层widget不可见，当上层widget也没有widgets时(最好是non-opaque, 这里没有进一步判断)，则可以直接拷贝新旧交叠区域。
    //non-opaque 时，滚动前后的下层widget发生了变化(其上的widget也可能变化)，因此只能全部重绘(并叠加)。
    bool accelerateScroll = accelEnv && isOpaque                                                                             //opaque paint event OR. opaque brush OR. paint on screen
                            && !(overlapped = isOverlapped(scrollRect.translated(data.crect.topLeft())));          //其上还有widget(可能是non-opaque的，这里并没有进一步判断)

    if (!accelerateScroll) {
        if (overlapped) {
            QRegion region(scrollRect);
            subtractOpaqueSiblings(region);
            invalidateBuffer(region);                                  //DHT 20150127, 递归绘制本widget 及其上的所有widget
        }else {
            invalidateBuffer(scrollRect);                             //DHT 20150127, 只绘制本widget
        }
    } else {
        ...
        QRegion childExpose(scrollRect);                       //DHT 20150127, 由于滚动而新暴露的区域
        if (sourceRect.isValid()) {
            if (wbs->bltRect(sourceRect, dx, dy, q))            //DHT 20141230,  可被 QPlatformBackingStore.scroll 加速(直接拷贝覆盖: src -> dst)
                childExpose -= destRect;
        }

        if (inDirtyList) {                                                    //DHT 20150127, 更新滚动后的dirty 区域
            if (rect == q->rect()) {
        ...
        if (!q->updatesEnabled())                                   //DHT 20150127, 不更新，滚动效果也看不到
            return;

        if (!childExpose.isEmpty()) {                                //DHT 20150127, 由于滚动而新暴露的区域，重绘
            wbs->markDirty(childExpose, q);                     //DHT 20150127, childExpose 已去掉新位置上的widget 区域，因此widget不会重绘
            isScrolled = true;
        }
        ...
        wbs->markDirtyOnScreen(destRect, q, toplevelOffset);               //DHT 20150127 触发flush
    }
}
...
//DHT 20140519, try cut down cpu time during qt updating
#if 1
        ...
        //DHT  toClean/beginPaint 用TLW coordinate
        ...
        } else {                                   //DHT  if there are some opaque widgets, toClean == dirty + wd->dirty, owtherwise, toClean == dirty
            resetWidget(w);
            dirty += widgetDirty;
        }
    }
    ...
    for (int i = 0; i < opaqueNonOverlappedWidgets.size(); ++i) {
        ...
        // Scrolled and moved widgets must draw all children.
        if (!wd->isScrolled && !wd->isMoved)
            flags |= QWidgetPrivate::DontDrawOpaqueChildren;                                 //DHT, this will cause paintSiblingsRecursive return immediately and NOT enter wd->drawWidgets, i.e., >>>NO recursion happened.<<<
        if (w == tlw)
            flags |= QWidgetPrivate::DrawAsRoot;

        QRegion toBePainted(wd->dirty);                                     // DHT, ref QWidgetBackingStore::markDirty
        resetWidget(w);

        QPoint offset(tlwOffset);
        if (w != tlw)
            offset += w->mapTo(tlw, QPoint());
        wd->drawWidget(store->paintDevice(), toBePainted, offset, flags, 0, this);     //DHT, recursive from opaque widget itself, but return from paintSiblingsRecursive immediately
    }

    // Paint the rest with composition.
    if (repaintAllWidgets || !dirtyCopy.isEmpty()) {
        const int flags = QWidgetPrivate::DrawAsRoot | QWidgetPrivate::DrawRecursive;
        tlw->d_func()->drawWidget(store->paintDevice(), dirtyCopy, tlwOffset, flags, 0, this);        //DHT, recirsive from tlw, >>>painter algorithm<<< cause it very slow.
    }

    endPaint(toClean, store, &beginPaintInfo);                                                        //DHT flush all dirtyOnScreen region(alien) and dirtyOnScreenWidgets(native)
}
...
/*!
    ...
    DHT widget 默认为0(从tlw flush, tlwOffset == 0)
*/
void QWidgetBackingStore::flush(QWidget *widget)
{
    if (!dirtyOnScreen.isEmpty()) {
        QWidget *target = widget ? widget : tlw;
        qt_flush(target, dirtyOnScreen, store, tlw, tlwOffset);               //DHT common case: widget == 0, ref. QWidgetBackingStore::endPaint
        dirtyOnScreen = QRegion();
    }

    if (!dirtyOnScreenWidgets || dirtyOnScreenWidgets->isEmpty())
        return;                                                               //DHT common case

    for (int i = 0; i < dirtyOnScreenWidgets->size(); ++i) {                  //DHT dirtyOnScreenWidgets 的2种情况: 1, 非tlw 的其它native widget; 2, 属于此tlw 的native widget(native child)
        QWidget *w = dirtyOnScreenWidgets->at(i);
        QWidgetPrivate *wd = w->d_func();
        Q_ASSERT(wd->needsFlush);
        qt_flush(w, *wd->needsFlush, store, tlw, tlwOffset);                  //DHT needsFlush 在 native widget 的坐标系, native widget 不是 tlw
