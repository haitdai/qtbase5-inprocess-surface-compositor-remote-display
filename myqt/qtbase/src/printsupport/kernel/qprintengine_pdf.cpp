...
//
//********************************************************************************************************************
bool QMDrawCmdPrintEngine::begin(QPaintDevice *pdev)
{
	return QMDrawCmdFileEngine::begin(pdev);
}

bool QMDrawCmdPrintEngine::end()
{ 
	return QMDrawCmdFileEngine::end();
}

QPrinter::PrinterState QMDrawCmdPrintEngine::printerState() const
{ 
	return QPrinter::Active; 
}

 bool QMDrawCmdPrintEngine::newPage()
 { 
 	return QMDrawCmdFileEngine::newPage(); 
 }
 
 int QMDrawCmdPrintEngine::metric(QPaintDevice::PaintDeviceMetric metric) const 
 { 
 	return QMDrawCmdFileEngine::metric(metric);
 }

void QMDrawCmdPrintEngine::setProperty(PrintEnginePropertyKey key, const QVariant &value)
{
	switch(key)
	{
		case QMDrawCmd_JobId:
			QMDrawCmdFileEngine::SetJobId( value.toString() );
			break;
			
		case QMDrawCmd_TotalPage:
			QMDrawCmdFileEngine::SetTotalPage( value.toUInt() );
			break;
			
		case QMDrawCmd_CurrentPage:
			QMDrawCmdFileEngine::SetCurrentPage( value.toUInt() );
			break;
			
		case QMDrawCmd_Rotate90:
			QMDrawCmdFileEngine::SetRotate90( value.toBool() );
			break;
			
		case QMDrawCmd_ShowSpeed:
			QMDrawCmdFileEngine::SetShowSpeed( value.toFloat() );
			break;
			
		case QMDrawCmd_FileType:
			QMDrawCmdFileEngine::SetFileType( value.toUInt() );
			break;

		case QMDrawCmd_ShowAreaSize:
			QMDrawCmdFileEngine::SetShowArea( value.toSize().width(), value.toSize().height());
			break;

		case QMDrawCmd_ShowAreaDpiWToH:
			QMDrawCmdFileEngine::SetShowAreaDpiWToH( value.toUInt() );
			break;
...
QVariant QMDrawCmdPrintEngine::property(PrintEnginePropertyKey key) const
{
	switch(key)
	{
		case QMDrawCmd_JobId:
			return QVariant((QString)((QMDrawCmdFileEngine *)this)->GetJobId());
			
		case QMDrawCmd_TotalPage:
			return QVariant((unsigned int)((QMDrawCmdFileEngine *)this)->GetCurrentPage());
			
		case QMDrawCmd_CurrentPage:
			return QVariant((unsigned int)((QMDrawCmdFileEngine *)this)->GetCurrentPage());
			
		case QMDrawCmd_Rotate90:
			return QVariant((bool)((QMDrawCmdFileEngine *)this)->GetRotate90());
			
		case QMDrawCmd_ShowSpeed:
			return QVariant((float)((QMDrawCmdFileEngine *)this)->GetShowSpeed());
			
		case QMDrawCmd_FileType:
			return QVariant((unsigned int)((QMDrawCmdFileEngine *)this)->GetFileType());

		case QMDrawCmd_CmdFileData:
			return QVariant((unsigned int)((QMDrawCmdFileEngine *)this)->GetCmdFileData());
