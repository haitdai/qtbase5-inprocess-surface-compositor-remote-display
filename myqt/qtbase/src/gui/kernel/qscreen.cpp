...
QScreen::QScreen(QPlatformScreen *screen, QPlatformCompositor *compositor)
    : QObject(*new QScreenPrivate(screen, compositor), 0)
{
}
#endif
...
/*!
    \brief get compositor of this screen
    \sa QPlatformIntegration::screenAdded
    \return under QMrOfs, return value is the platformCompositor which is bound to this screen
    \return under QMrOfsV2, return value is NULL, caller can call QWindow.compositor to get correct in-win compositor
    \note deprecated under QMrOfsV2
*/
#ifdef QT_MROFS_LEGACY
QPlatformCompositor *QScreen::compositor() const
{
    Q_D(const QScreen);
    return d->platformCompositor;
}
#endif
...
{
    Q_D(const QScreen);
    return d->geometry;            //DHT 20141120, ref. QScreenPrivate.geometry, QMrOfsScreen.geometry()
}
...
Qt::ScreenOrientation QScreen::setOrientation(Qt::ScreenOrientation orientation)
{
    Q_D(const QScreen);
    return d->platformScreen->setOrientation(orientation);
}
QT_END_NAMESPACE
