...
/*!
    !!! inputmgr thread !!! for SMOOT mouse cursor moving which should not be disturbed by cpu-exhuasted work in ui thread...
    under qt's desigin law, all other handleXXX are in ui thread except this one
*/
void QWindowSystemInterface::handleMouseCursorMoveEvent(QWindow *w, const QPointF & local, const QPointF & global, Qt::MouseButtons b, Qt::KeyboardModifiers mods)
{
    unsigned long timestamp = QWindowSystemInterfacePrivate::eventTime.elapsed();
    QWindowSystemInterfacePrivate::MouseCursorMoveEvent e(w, timestamp, local, global, b, mods);          
    QGuiApplicationPrivate::processWindowSystemEvent(&e);  
}

/*!
\a w == 0 means that the event is in global coords only, \a local will be ignored in this case
\if delivered by inputmgr or evdev, local == global == primary screen coordinate, and w == NULL
\if delivered by xcb, local == window coordinate, global == screen coordinate, and window != NULL
*/
