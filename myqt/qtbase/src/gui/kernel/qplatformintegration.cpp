...
{
    QScreen *screen = new QScreen(ps);                //DHT new QScreen which backend is QPlatformScreen(such as QXcbScreen)
...
void QPlatformIntegration::screenAdded(QPlatformScreen *ps, QPlatformCompositor *compositor)
{
    QScreen *screen = new QScreen(ps, compositor);  //DHT new QScreen which backend is QPlatformScreen(such as QXcbScreen), effective version: compositor
    ps->d_func()->screen = screen;
    QGuiApplicationPrivate::screen_list << screen;
    emit qGuiApp->screenAdded(screen);
}
