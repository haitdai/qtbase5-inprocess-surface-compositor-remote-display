...
#ifdef QT_PRINTFILE
#include "../../next_print_file/Include/MM_Types.h"
#include "../../next_print_file/Include/PrintFile.h"
#endif
...
class Q_GUI_EXPORT QMDrawCmdFileEngine : public QPaintEngine
{
public:
    	QMDrawCmdFileEngine();
    	~QMDrawCmdFileEngine();
	...
	void SetTotalPage(DWORD totalPage);
	DWORD GetTotalPage();

	void SetCurrentPage(DWORD currentPage);
	DWORD GetCurrentPage();

	VOID SetRotate90(BOOL bRotate90);
	BOOL GetRotate90();

	VOID SetShowSpeed(FLOAT speed);
	FLOAT GetShowSpeed();

	VOID SetFileType( UCHAR  fileType );
	UCHAR GetFileType();

	VOID SetShowArea( int width, int height );
	VOID SetShowAreaDpiWToH( int dpiWToH );

	MPrintFileInterface::CmdFileData * GetCmdFileData();
	UINT16 m_WHDpiRatio;

private:
	MPrintFileInterface * m_pPrintFileCreate;
};
#endif//QT_PRINTFILE
