...
inline void QBlitterPaintEnginePrivate::lock()
{
    if (!pmData->blittable()->isLocked())
        rasterBuffer->prepare(pmData->buffer());   //DHT 20141230 pmData->buffer() 隐含了 blittable()->lock()
}
...
//DHT 20141230 ，这个函数是支持 clip region/ clip rect 的，将region 转换成对blittable 的rect 操作
...
{
    Q_D(QBlitterPaintEngine);
    if (d->caps.canBlitterAlphaFillRect()) {         //DHT 20141230, 支持 alpha fill 加速
        d->fillRect(rect, color, true);
    } else if (d->caps.canBlitterFillRect() && color.alpha() == 0xff) {         //DHT 20141230 ，支持非 alpha fill 加速且 color.alpha == 0xFF
        d->fillRect(rect, color, false);
    } else {                                                     //DHT 20141230, 转纯软件实现
        d->lock();
...
void QBlitterPaintEngine::fillRect(const QRectF &rect, const QBrush &brush)
{
    ...
    if (qbrush_style(brush) == Qt::SolidPattern
        ...
        d->unlock();               //DHT 20141230 可能用到硬件加速，总是先 unlock，如果硬件加速测试失败，再 lock
...
//DHT 把 pm 画到 blitterPaintEngine 内部的pixmap 上
//区别于 QBlitterPaintEngine::drawImage
