...
void QRasterPaintEngine::init()
{
    ...
    case QInternal::Image:
        format = d->rasterBuffer->prepare(static_cast<QImage *>(d->device));            /*<DHT, 20140714, set backingstore buffer into paint engine*/
        break;
    default:
...
bool QRasterPaintEngine::begin(QPaintDevice *device)
{
    ...
    if (device->devType() == QInternal::Pixmap) {
        ...
        if (pd->classId() == QPlatformPixmap::RasterClass || pd->classId() == QPlatformPixmap::BlitterClass)
            d->device = pd->buffer();         //可能是直接map/lock 的硬件表面, ref . QBlittablePlatformPixmap::buffer()
    } else {
        d->device = device;
    }
...
void QRasterPaintEnginePrivate::updateMatrixData(QSpanData *spanData, const QBrush &b, const QTransform &m)
{
    ...
    if (b.d->transform.type() > QTransform::TxNone) { // FALCON: optimize
        ...
        if (m.type() <= QTransform::TxTranslate) {       // TxNone or      TxTranslate
                                ...
                                QRasterPaintEnginePrivate *pe)
{
    ...
    while (y < y2) {
        ...
        while (i < n) {
            ...
            spans[i].len = width;
            spans[i].y = y + i;
            spans[i].coverage = 255;                  //DHT20141113, no blending required if antialiasing flag is unset
            ++i;
        }
...
void QRasterPaintEngine::fillRect(const QRectF &r, QSpanData *data)     // r: TLW coord
{
    Q_D(QRasterPaintEngine);
    QRasterPaintEngineState *s = state();

    //DHT20141121, 取整填充，比较快，小数部分可能有截断误差。
    if (!s->flags.antialiased) {
        //qWarning("!!!!!!!!!!!!!!!non-antialiasing fillRect");
    ...
    //DHT20141121, 光栅化，抗锯齿，比较慢
    ensureRasterState();
    if (s->flags.tx_noshear) {
        d->initializeRasterizer(data);
        //qWarning("!!!!!!!!!!!!!r(%f, %f, %f, %f)", r.x(), r.y(), r.width(), r.height());
        QRectF nr = r.normalized();
//        qWarning("!!!!!!!!!!!!!nr(%f, %f, %f, %f)", nr.x(), nr.y(), nr.width(), nr.height());        
        if (!nr.isEmpty()) {
    ...
    //DHT20141121，有shear transform, 按path 填充，可能最慢，TTF 可能也是这样画出的。
    //qWarning("!!!!!!!!!!!!shear fillRect");
    QPainterPath path;
    path.addRect(r);
    ensureOutlineMapper();
    fillPath(path, data);
...
void QRasterPaintEngine::fillRect(const QRectF &r, const QBrush &brush)
{
    ...
    fillRect(r, &s->brushData);         // QSpanData
}
...
void QRasterPaintEngine::fillRect(const QRectF &r, const QColor &color)
{
    ...
    //DHT
    //qWarning("d->solid_color_filler.solid.color(%x)", d->solid_color_filler.solid.color);
    //END
    d->solid_color_filler.clip = d->clip();
    d->solid_color_filler.adjustSpanMethods();
...
/*!
    \internal
*/
//DHT 20141230, 把 img 的部分区域画到 this->rasterBuffer 中
                                   ...
                                   Qt::ImageConversionFlags)
{
    ...
    if (s->matrix.type() <= QTransform::TxScale && !s->flags.antialiased && sr_l == sr_r && sr_t == sr_b) {
        ...
        d->solid_color_filler.clip = d->clip();
        d->solid_color_filler.adjustSpanMethods();
        fillRect(r, &d->solid_color_filler);                                                                                                  //DHT20141114, if width is zero, use fillRect directly
                ...
                && d->rasterBuffer->compositionMode == QPainter::CompositionMode_Source)))
    {
        ...
        if (rotationType != NoRotation && qMemRotateFunctions[d->rasterBuffer->format][rotationType] && img.rect().contains(sr.toAlignedRect())) {
                ...
                || (isPixelAligned(transformedTargetRect) && isPixelAligned(sr)))
            {
                ...
                qMemRotateFunctions[d->rasterBuffer->format][rotationType](srcBase, cw, ch, sbpl, dstBase, dbpl);                    //DHT20141113, memrotate.cpp
    ...
    if (s->flags.fast_text) {
        if (unclipped) {
            if (depth == 1) {
                ...
                if (s->penData.alphamapBlit) {            //DHT ref. qt_alphamapblit_quint32
...
void QRasterPaintEngine::drawTextItem(const QPointF &p, const QTextItem &textItem)
{
    ...
    //DHT 20150202 对于raster paintengine 仍然是cache较小的glyph
...
// m_buffer is the entire image of backingstore(or other paint device)
QImage::Format QRasterBuffer::prepare(QImage *image)
{
    ...
    //DHT
    //qDebug("[!QRasterBuffer::prepare] image.bits: 0x%x, width: %d, height: %d, bytes_per_pixel: %d", m_buffer, m_width, m_height, bytes_per_pixel);
    //END    
